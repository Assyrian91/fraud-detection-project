...
NW_TO_PD_DTYPES_BACKEND: Mapping[
    ...
    dtypes.String: {
        "pyarrow": "string[pyarrow]",
        "numpy_nullable": "string",
        None: str,
    },
    dtypes.Boolean: {
...
def narwhals_to_native_dtype(  # noqa: C901, PLR0912
    ...
    if issubclass(
        base_type,
        (dtypes.Struct, dtypes.Array, dtypes.List, dtypes.Time, dtypes.Binary),
    ):
...
def narwhals_to_native_arrow_dtype(
    ...
    if is_pandas_or_modin(implementation) and PANDAS_VERSION >= (2, 2):
        try:
            ...
            msg = (
                f"Unable to convert to {dtype} to to the following exception: {exc.msg}"
            )
