...
class OverrideParser(Parser):
    ...
    def override(self):
        ...
        try:
            ...
            if token in [
                OverrideParser.EQUAL,
                OverrideParser.AT,
                OverrideParser.KEY_SPECIAL,
                OverrideParser.DOT_PATH,
                OverrideParser.ID,
            ]:
                self.state = 28
                self.key()
                ...
                if ((_la) & ~0x3F) == 0 and (
                    (1 << _la)
                    & (
                        (1 << OverrideParser.COLON)
                        | (1 << OverrideParser.BRACKET_OPEN)
                        | (1 << OverrideParser.BRACE_OPEN)
                        | (1 << OverrideParser.FLOAT)
                        | (1 << OverrideParser.INT)
                        | (1 << OverrideParser.BOOL)
                        | (1 << OverrideParser.NULL)
                        | (1 << OverrideParser.UNQUOTED_CHAR)
                        | (1 << OverrideParser.ID)
                        | (1 << OverrideParser.ESC)
                        | (1 << OverrideParser.WS)
                        | (1 << OverrideParser.QUOTED_VALUE)
                        | (1 << OverrideParser.INTERPOLATION)
                    )
                ) != 0:
                    self.state = 30
                    self.value()
        ...
        def dictKeyValuePair(self, i: int = None):
            if i is None:
                ...
                return self.getTypedRuleContext(
                    OverrideParser.DictKeyValuePairContext, i
                )
    ...
    def primitive(self):
        ...
        try:
            ...
            if token in [OverrideParser.QUOTED_VALUE]:
                ...
                OverrideParser.COLON,
                OverrideParser.FLOAT,
                OverrideParser.INT,
                OverrideParser.BOOL,
                OverrideParser.NULL,
                OverrideParser.UNQUOTED_CHAR,
                OverrideParser.ID,
                OverrideParser.ESC,
                OverrideParser.WS,
                OverrideParser.INTERPOLATION,
            ]:
                self.enterOuterAlt(localctx, 2)
                self.state = 147
                self._errHandler.sync(self)
