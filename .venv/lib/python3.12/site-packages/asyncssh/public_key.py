...
_CertAlgMap = Dict[bytes, Tuple[Optional[Type["SSHKey"]], Type["SSHCertificate"]]]
_CertSigAlgMap = Dict[bytes, bytes]
_CertVersionMap = Dict[Tuple[bytes, int], Tuple[bytes, Type["SSHOpenSSHCertificate"]]]
...
_OpenSSHCertParams = Tuple[object, int, int, bytes, bytes, int, int, bytes, bytes]
...
_subject_pattern = re.compile(
    r"(?:Distinguished[ -_]?Name|Subject|DN)[=:]?\s?", re.IGNORECASE
)
...
class SSHKey:
    ...
    def _generate_certificate(
        ...
        if valid_before <= valid_after:
            raise ValueError("Valid before time must be later than " "valid after time")
        ...
        try:
            algorithm, cert_handler = _certificate_version_map[key.algorithm, version]
        except KeyError:
            raise KeyGenerationError("Unknown certificate version") from None

        return cert_handler.generate(
            self,
            algorithm,
            key,
            serial,
    ...
    def _generate_x509_certificate(
        self,
        key: "SSHKey",
        subject: str,
        issuer: Optional[str],
        ...
        if not _x509_available:  # pragma: no cover
            raise KeyGenerationError(
                "X.509 certificate generation " "requires PyOpenSSL"
            )

        if not self.x509_algorithms:
            raise KeyGenerationError(
                "X.509 certificate generation not "
                "supported for " + self.get_algorithm() + " keys"
            )
        ...
        return SSHX509Certificate.generate(
            self,
            key,
            subject,
            issuer,
    ...
    def set_comment(
        self, comment: _Comment, encoding: str = "utf-8", errors: str = "strict"
    ) -> None:
    ...
    def sign(self, data: bytes, sig_algorithm: bytes) -> bytes:
        ...
        return b"".join((String(sig_algorithm), self.sign_ssh(data, sig_algorithm)))
    ...
    def generate_user_certificate(
        ...
        if source_address:
            cert_options["source-address"] = [
                ip_network(addr) for addr in source_address
            ]
    ...
    def generate_x509_ca_certificate(
        self,
        ca_key: "SSHKey",
        subject: str,
        issuer: Optional[str] = None,
    ...
    def export_private_key(
        ...
        if format_name in ("pkcs1-der", "pkcs1-pem"):
            ...
            if passphrase is not None:
                if format_name == "pkcs1-der":
                    raise KeyExportError(
                        "PKCS#1 DER format does not support " "private key encryption"
                    )
            ...
            if passphrase is not None:
                data = pkcs8_encrypt(
                    data, cipher_name, hash_name, pbe_version, passphrase
                )
            ...
            data = b"".join(
                (check, check, self.private_data, String(self._comment or b""))
            )
            ...
            if passphrase is not None:
                try:
                    ...
                    key_size, iv_size, block_size, _, _, _ = get_encryption_params(alg)
                except (KeyError, UnicodeEncodeError):
                    raise KeyEncryptionError("Unknown cipher: " + cipher_name) from None
                ...
                key = bcrypt.kdf(
                    passphrase, salt, key_size + iv_size, rounds, ignore_few_rounds
                )
...
class SSHCertificate:
    ...
    def __eq__(self, other: object) -> bool:
        return isinstance(other, type(self)) and self.public_data == other.public_data
    ...
    def export_certificate(self, format_name: str = "openssh") -> bytes:
        ...
        if self.is_x509:
            if format_name == "rfc4716":
                raise KeyExportError(
                    "RFC4716 format is not supported for " "X.509 certificates"
                )
        else:
            if format_name in ("der", "pem"):
                raise KeyExportError(
                    "DER and PEM formats are not supported " "for OpenSSH certificates"
                )
    ...
    def write_certificate(
        self, filename: FilePath, format_name: str = "openssh"
    ) -> None:
...
class SSHOpenSSHCertificate(SSHCertificate):
    ...
    def generate(
        ...
        if cert_type == CERT_TYPE_USER:
            cert_options = cls._encode_options(options, cls._user_option_encoders)
            cert_extensions = cls._encode_options(options, cls._user_extension_encoders)
        else:
            cert_options = cls._encode_options(options, cls._host_option_encoders)
            cert_extensions = cls._encode_options(options, cls._host_extension_encoders)
    ...
    def construct(
        ...
        (
            ...
            options,
            extensions,
        ) = cls._decode(packet, key_handler)
        ...
        while packet:
            try:
                ...
                raise KeyImportError(
                    "Invalid characters in principal " "name"
                ) from None

            principals.append(principal)

        if cert_type == CERT_TYPE_USER:
            cert_options = cls._decode_options(options, cls._user_option_decoders, True)
            cert_options.update(
                cls._decode_options(extensions, cls._user_extension_decoders, False)
            )
        elif cert_type == CERT_TYPE_HOST:
            cert_options = cls._decode_options(options, cls._host_option_decoders, True)
            cert_options.update(
                cls._decode_options(extensions, cls._host_extension_decoders, False)
            )
        else:
    ...
    @classmethod
    def _decode(
        cls, packet: SSHPacket, key_handler: Type[SSHKey]
    ) -> _OpenSSHCertParams:
    ...
    @staticmethod
    def _encode_options(
        options: _OpenSSHCertOptions, encoders: _OpenSSHCertEncoders
    ) -> bytes:
    ...
    def _decode_source_addr(packet: SSHPacket) -> Sequence[IPNetwork]:
        ...
        try:
            return [ip_network(addr.decode("ascii")) for addr in packet.get_namelist()]
        except (UnicodeDecodeError, ValueError):
            raise KeyImportError("Invalid source address") from None

    @staticmethod
    def _decode_options(
        options: bytes, decoders: _OpenSSHCertDecoders, critical: bool = True
    ) -> _OpenSSHCertOptions:
...
class SSHX509Certificate(SSHCertificate):
    ...
    def __init__(
        self, key: SSHKey, x509_cert: "X509Certificate", comment: _Comment = None
    ):
        super().__init__(
            b"x509v3-" + key.algorithm,
            key.x509_algorithms,
            key.x509_algorithms,
            key,
            x509_cert.data,
    ...
    def _expand_trust_store(
        self,
        cert: "SSHX509Certificate",
        trusted_cert_paths: Sequence[FilePath],
        trust_store: Set["SSHX509Certificate"],
    ) -> None:
    ...
    def generate(
        ...
        x509_cert = generate_x509_certificate(
            signing_key.pyca_key,
            key.pyca_key,
            subject,
            issuer,
            serial,
    ...
    @classmethod
    def construct_from_der(
        cls, data: bytes, comment: _Comment = None
    ) -> "SSHX509Certificate":
    ...
    def validate_chain(
        self,
        trust_chain: Sequence["SSHX509Certificate"],
        ...
        trust_store = {c for c in trust_chain if c.subject != c.issuer} | set(
            trusted_certs
        )
        ...
        self.x509_cert.validate(
            [c.x509_cert for c in trust_store], purposes, user_principal, host_principal
        )
...
class SSHX509CertificateChain(SSHCertificate):
    ...
    def __init__(
        ...
        super().__init__(
            algorithm, key.x509_algorithms, key.x509_algorithms, key, data, comment
        )
    ...
    def construct(
        ...
        certs = [import_certificate(packet.get_string()) for _ in range(cert_count)]
    ...
    @classmethod
    def construct_from_certs(
        cls, certs: Sequence["SSHCertificate"]
    ) -> "SSHX509CertificateChain":
        ...
        if revoked_certs:
            for cert in self._certs:
                if cert in revoked_certs:
                    raise ValueError(
                        "Revoked X.509 certificate in " "certificate chain"
                    )

        self._certs[0].validate_chain(
            self._certs[1:],
            trusted_certs,
            trusted_cert_paths,
            purposes,
...
class SSHLocalKeyPair(SSHKeyPair):
    ...
    def get_agent_private_key(self) -> bytes:
        ...
        if self._cert:
            data = String(self.public_data) + self._key.encode_agent_cert_private()
        else:
...
def _parse_openssh(data: bytes) -> Tuple[bytes, Optional[bytes], bytes]:
    ...
    if line[0] not in _public_key_alg_map and line[0] not in _certificate_alg_map:
        raise KeyImportError("Unknown OpenSSH public key algorithm")

    try:
        return line[0], comment, binascii.a2b_base64(line[1])
    except binascii.Error:
        raise KeyImportError("Invalid OpenSSH public key " "or certificate") from None
...
def _match_block(data: bytes, start: int, header: bytes, fmt: str) -> Tuple[bytes, int]:
    ...
    match = re.compile(
        b"^" + header[:5] + b"END" + header[10:] + rb"[ \t\r\f\v]*$", re.M
    ).search(data, start)
...
def _match_next(
    data: bytes, keytype: bytes, public: bool = False
    ...
    while end != 0:
        ...
        if line.startswith(b"-----BEGIN ") and line.endswith(b" " + keytype + b"-----"):
...
def _decode_pkcs1_private(
    pem_name: bytes, key_data: object, unsafe_skip_rsa_key_validation: Optional[bool]
) -> SSHKey:
    """Decode a PKCS#1 format private key"""

    handler = _pem_map.get(pem_name)
    if handler is None:
        raise KeyImportError("Unknown PEM key type: " + pem_name.decode("ascii"))

    key_params = handler.decode_pkcs1_private(key_data)
    if key_params is None:
        raise KeyImportError(f'Invalid {pem_name.decode("ascii")} private key')

    if pem_name == b"RSA":
        key_params = cast(Tuple, key_params) + (unsafe_skip_rsa_key_validation,)
...
def _decode_pkcs8_private(
    key_data: object, unsafe_skip_rsa_key_validation: Optional[bool]
) -> SSHKey:
    """Decode a PKCS#8 format private key"""

    if (
        isinstance(key_data, tuple)
        and len(key_data) >= 3
        and key_data[0] in (0, 1)
        and isinstance(key_data[1], tuple)
        and 1 <= len(key_data[1]) <= 2
        and isinstance(key_data[2], bytes)
    ):
        if len(key_data[1]) == 2:
        ...
        if key_params is None:
            key_type = (
                handler.pem_name.decode("ascii") if handler.pem_name else "PKCS#8"
            )
...
def _decode_pkcs8_public(key_data: object) -> SSHKey:
    ...
    if (
        isinstance(key_data, tuple)
        and len(key_data) == 2
        and isinstance(key_data[0], tuple)
        and 1 <= len(key_data[0]) <= 2
        and isinstance(key_data[1], BitString)
        and key_data[1].unused == 0
    ):
        if len(key_data[0]) == 2:
...
def _decode_openssh_private(
    ...
    try:
        ...
        if cipher_name != b"none":
            ...
            try:
                key_size, iv_size, _, _, _, _ = get_encryption_params(cipher_name)
            except KeyError:
                raise KeyEncryptionError(
                    "Unknown cipher: " + cipher_name.decode("ascii")
                ) from None
            ...
            try:
                bcrypt_key = bcrypt.kdf(
                    passphrase, salt, key_size + iv_size, rounds, ignore_few_rounds=True
                )
            except ValueError:
                raise KeyEncryptionError("Invalid OpenSSH " "private key") from None

            cipher = get_encryption(
                cipher_name, bcrypt_key[:key_size], bcrypt_key[key_size:]
            )
...
def _decode_der_private(
    ...
    for pem_name in _pem_map:
        try:
            return _decode_pkcs1_private(
                pem_name, key_data, unsafe_skip_rsa_key_validation
            )
        except KeyImportError:
...
def _decode_der_certificate(data: bytes, comment: _Comment = None) -> SSHCertificate:
...
def _decode_pem_private(
    ...
    if pem_name == b"OPENSSH":
        return _decode_openssh_private(data, passphrase, unsafe_skip_rsa_key_validation)
    ...
    if headers.get(b"Proc-Type") == b"4,ENCRYPTED":
        ...
        try:
            ...
            raise KeyImportError("Unable to decrypt PKCS#1 " "private key") from None
    ...
    if pem_name == b"ENCRYPTED":
        ...
        try:
            ...
            raise KeyImportError("Unable to decrypt PKCS#8 " "private key") from None

    if pem_name:
        return _decode_pkcs1_private(pem_name, key_data, unsafe_skip_rsa_key_validation)
    else:
        return _decode_pkcs8_private(key_data, unsafe_skip_rsa_key_validation)
...
def _decode_private(
    ...
    if fmt == "der":
        key = _decode_der_private(
            key_info[0], passphrase, unsafe_skip_rsa_key_validation
        )
    elif fmt == "pem":
        pem_name, headers, data = key_info
        key = _decode_pem_private(
            pem_name, headers, data, passphrase, unsafe_skip_rsa_key_validation
        )
    else:
        key = None
...
def _decode_certificate(data: bytes) -> Tuple[Optional[SSHCertificate], Optional[int]]:
...
def _decode_private_list(
    ...
    while data:
        key, end = _decode_private(data, passphrase, unsafe_skip_rsa_key_validation)
...
def register_public_key_alg(
    algorithm: bytes,
    handler: Type[SSHKey],
    default: bool,
    sig_algorithms: Optional[Sequence[bytes]] = None,
) -> None:
...
def register_certificate_alg(
    ...
    _certificate_version_map[algorithm, version] = (cert_algorithm, cert_handler)
...
def decode_ssh_public_key(data: bytes) -> SSHKey:
    ...
    try:
        ...
        if handler:
            ...
            raise KeyImportError(
                "Unknown key algorithm: " + alg.decode("ascii", errors="replace")
            )
    except PacketDecodeError:
        raise KeyImportError("Invalid public key") from None


def decode_ssh_certificate(data: bytes, comment: _Comment = None) -> SSHCertificate:
...
def generate_private_key(alg_name: str, comment: _Comment = None, **kwargs) -> SSHKey:
...
def read_private_key(
    ...
    key = import_private_key(
        read_file(filename), passphrase, unsafe_skip_rsa_key_validation
    )
...
def read_private_key_and_certs(
    ...
    key, cert = import_private_key_and_certs(
        read_file(filename), passphrase, unsafe_skip_rsa_key_validation
    )
...
def load_keypairs(
        ...
        try:
            ...
            if loop and inspect.isawaitable(resolved_passphrase):
                resolved_passphrase = asyncio.run_coroutine_threadsafe(
                    resolved_passphrase, loop
                ).result()

            priv_keys = read_private_key_list(
                keylist, resolved_passphrase, unsafe_skip_rsa_key_validation
            )
    ...
    for key_to_load in keys_to_load:
        ...
        try:
            if isinstance(key_to_load, (PurePath, str)):
                ...
                if allow_certs:
                    key, certs_to_load = read_private_key_and_certs(
                        key_to_load, resolved_passphrase, unsafe_skip_rsa_key_validation
                    )
                ...
                if allow_certs:
                    key, certs_to_load = import_private_key_and_certs(
                        key_to_load, passphrase, unsafe_skip_rsa_key_validation
                    )
                else:
                    key = import_private_key(
                        key_to_load, passphrase, unsafe_skip_rsa_key_validation
            ...
            if skip_public or (ignore_encrypted and str(exc).startswith("Passphrase")):
                continue
...
def load_default_keypairs(
    passphrase: Optional[BytesOrStr] = None, certlist: CertListArg = ()
) -> Sequence[SSHKeyPair]:
    ...
    for file, condition in _DEFAULT_KEY_FILES:
        if condition:  # pragma: no branch
            try:
                ...
                result.extend(
                    load_keypairs(path, passphrase, certlist, ignore_encrypted=True)
                )
            except OSError:
                pass
...
def load_identities(
    keylist: IdentityListArg, skip_private: bool = False
) -> Sequence[bytes]:
