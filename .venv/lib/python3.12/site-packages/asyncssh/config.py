...
def _exec(cmd: str) -> bool:
    ...
    return (
        subprocess.run(
            cmd, check=False, shell=True, stdin=DEVNULL, stdout=DEVNULL, stderr=DEVNULL
        ).returncode
        == 0
    )
...
class SSHConfig:
    ...
    def __init__(
        self,
        last_config: Optional["SSHConfig"],
        reload: bool,
        canonical: bool,
        final: bool,
    ):
    ...
    def _expand_token(self, match):
        ...
        try:
            ...
            if token == "d":
                raise ConfigParseError("Home directory is " "not available") from None
            elif token == "i":
                raise ConfigParseError("User id not available") from None
            else:
                raise ConfigParseError("Invalid token expansion: " + token) from None
    ...
    def _expand_env(match):
        ...
        try:
            ...
            raise ConfigParseError("Invalid environment expansion: " + var) from None

    def _expand_val(self, value: str) -> str:
        """Perform percent token and environment expansion on a string"""

        return _env_pattern.sub(
            self._expand_env, _token_pattern.sub(self._expand_token, value)
        )
            ...
            if match == "all":
                ...
                if matching:
                    if match == "exec":
                        ...
                        ip = ip_address(cast(str, match_val)) if match_val else None
...
class SSHClientConfig(SSHConfig):
    ...
    def _set_hostname(self, option: str, args: List[str]) -> None:
        ...
        if option not in self._options:
            self._tokens["h"] = cast(str, self._options.get(option, self._orig_host))
    ...
    _handlers = {
        option.lower(): (option, handler)
        for option, handler in (
            ("Host", _match_host),
