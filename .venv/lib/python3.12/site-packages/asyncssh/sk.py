...
def _ctap1_poll(
    poll_interval: float, func: Callable[..., _PollResult], *args: object
) -> _PollResult:
...
def _ctap1_enroll(
    dev: "CtapHidDevice", alg: int, application: str
) -> Tuple[bytes, bytes]:
    ...
    registration = _ctap1_poll(
        _CTAP1_POLL_INTERVAL, ctap1.register, _dummy_hash, app_hash
    )
...
def _ctap1_sign(
    dev: "CtapHidDevice", message_hash: bytes, application: str, key_handle: bytes
) -> Tuple[int, int, bytes]:
    ...
    auth_response = _ctap1_poll(
        _CTAP1_POLL_INTERVAL, ctap1.authenticate, message_hash, app_hash, key_handle
    )
...
def _ctap2_sign(
    ...
    if touch_required:
        ctap2.get_assertions(
            application, message_hash, allow_creds, options={"up": False}
        )

    assertion = ctap2.get_assertions(
        application, message_hash, allow_creds, options=options
    )[0]
...
def _win_sign(
    data: bytes, application: str, key_handle: bytes
    ...
    options = {"challenge": data, "rpId": application, "allowCredentials": creds}
...
def sk_webauthn_prefix(data: bytes, application: str) -> bytes:
    ...
    return (
        ...
        + urlsafe_b64encode(data).rstrip(b"=")
        + b'","origin":"'
...
def sk_enroll(
    alg: int, application: str, user: str, pin: Optional[str], resident: bool
) -> Tuple[bytes, bytes]:
...
def sk_sign(
    ...
    for dev in CtapHidDevice.list_devices():
        try:
            flags, counter, sig = _ctap2_sign(
                dev, message_hash, application, key_handle, touch_required
            )
...
def sk_get_resident(
    application: str, user: Optional[str], pin: str
) -> Sequence[_SKResidentKey]:
    ...
    for dev in CtapHidDevice.list_devices():
        try:
            ...
            for cred in cred_mgmt.enumerate_creds(app_hash):
                user_info = cast(
                    Mapping[str, object], cred[CredentialManagement.RESULT.USER]
                )
                ...
                public_key = cast(
                    Mapping[int, object], cred[CredentialManagement.RESULT.PUBLIC_KEY]
                )
