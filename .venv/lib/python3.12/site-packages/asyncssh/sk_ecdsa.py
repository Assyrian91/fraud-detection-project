...
class _SKECDSAKey(SSHKey):
    ...
    def __eq__(self, other: object) -> bool:
        ...
        return (
            ...
            and self._key.curve_id == other._key.curve_id
            and self._key.public_value == other._key.public_value
            and self._application == other._application
            and self._flags == other._flags
            and self._key_handle == other._key_handle
    ...
    def generate(
        ...
        *,  # type: ignore
        ...
        public_value, key_handle = sk_enroll(
            SSH_SK_ECDSA, application, user, pin, resident
        )

        # Strip prefix and suffix of algorithm to get curve_id
        return cls(algorithm[14:-12], public_value, application, flags, key_handle, b"")

    @classmethod
    def make_private(cls, key_params: object) -> SSHKey:
        """Construct a U2F ECDSA private key"""

        curve_id, public_value, application, flags, key_handle, reserved = cast(
            _PrivateKeyArgs, key_params
        )

        return cls(curve_id, public_value, application, flags, key_handle, reserved)
    ...
    def encode_ssh_private(self) -> bytes:
        ...
        return b"".join(
            (
                String(self._key.curve_id),
                String(self._key.public_value),
                String(self._application),
                Byte(self._flags),
                String(self._key_handle),
                String(self._reserved),
            )
        )
    ...
    def sign_ssh(self, data: bytes, sig_algorithm: bytes) -> bytes:
        ...
        flags, counter, sig, client_data = sk_sign(
            data, self._application, self._key_handle, self._flags, is_webauthn
        )
    ...
    def verify_ssh(self, data: bytes, sig_algorithm: bytes, packet: SSHPacket) -> bool:
        ...
        return self._key.verify(
            self._app_hash + Byte(flags) + UInt32(counter) + sha256(data).digest(),
            sig,
            "sha256",
        )
...
register_public_key_alg(
    _algorithm, _SKECDSAKey, True, (_algorithm, b"webauthn-" + _algorithm)
)

register_certificate_alg(
    1, _algorithm, _cert_algorithm, _SKECDSAKey, SSHOpenSSHCertificateV01, True
)
