...
class Auth(SSHPacketHandler):
    ...
    def send_packet(self, pkttype: int, *args: bytes, trivial: bool = True) -> None:
        """Send an auth packet"""

        self._conn.send_userauth_packet(pkttype, *args, handler=self, trivial=trivial)
...
class ClientAuth(Auth):
    ...
    async def send_request(
        self, *args: bytes, key: Optional[SigningKey] = None, trivial: bool = True
    ) -> None:
        """Send a user authentication request"""

        await self._conn.send_userauth_request(
            self._method, *args, key=key, trivial=trivial
        )
...
class _ClientGSSKexAuth(ClientAuth):
    ...
    async def _start(self) -> None:
        ...
        if self._conn.gss_kex_auth_requested():
            ...
            await self.send_request(key=self._conn.get_gss_context(), trivial=False)
        else:
...
class _ClientGSSMICAuth(ClientAuth):
    ...
    def _finish(self) -> None:
        ...
        if self._gss.provides_integrity:
            ...
            self.send_packet(
                MSG_USERAUTH_GSSAPI_MIC, String(self._gss.sign(data)), trivial=False
            )
        else:
    ...
    async def _process_error_token(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
...
class _ClientHostBasedAuth(ClientAuth):
    ...
    async def _start(self) -> None:
        ...
        self.logger.debug1(
            "Trying host based auth of user %s on host %s " "with %s host key",
            client_username,
            client_host,
            keypair.algorithm,
        )
...
class _ClientPublicKeyAuth(ClientAuth):
    ...
    async def _start(self) -> None:
        ...
        self.logger.debug1(
            "Trying public key auth with %s key", self._keypair.algorithm
        )
    ...
    async def _send_signed_request(self) -> None:
        ...
        self.logger.debug1("Signing request with %s key", self._keypair.algorithm)
    ...
    _packet_handlers = {MSG_USERAUTH_PK_OK: _process_public_key_ok}
...
class _ClientKbdIntAuth(ClientAuth):
    ...
    async def _receive_challenge(
        self, name: str, instruction: str, lang: str, prompts: KbdIntPrompts
    ) -> None:
        """Receive and respond to a keyboard interactive challenge"""

        responses = await self._conn.kbdint_challenge_received(
    ...
    def _process_info_request(
        ...
        self.create_task(self._receive_challenge(name, instruction, lang, prompts))

    _packet_handlers = {MSG_USERAUTH_INFO_REQUEST: _process_info_request}
...
class _ClientPasswordAuth(ClientAuth):
    ...
    async def _start(self) -> None:
        ...
        await self.send_request(Boolean(False), String(password), trivial=False)
    ...
    _packet_handlers = {MSG_USERAUTH_PASSWD_CHANGEREQ: _process_password_change}
...
class _ServerGSSKexAuth(ServerAuth):
    ...
    async def _start(self, packet: SSHPacket) -> None:
        ...
        if (
            self._gss.complete
            and self._gss.verify(data, mic)
            and (
                await self._conn.validate_gss_principal(
                    self._username, self._gss.user, self._gss.host
                )
            )
        ):
...
class _ServerGSSMICAuth(ServerAuth):
    ...
    async def _finish(self) -> None:
        ...
        if await self._conn.validate_gss_principal(
            self._username, self._gss.user, self._gss.host
        ):
            await self.send_success()
        else:
    ...
    def _process_exchange_complete(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
    ...
    def _process_mic(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
        ...
        if (
            self._gss.complete
            and self._gss.provides_integrity
            and self._gss.verify(data, mic)
        ):
            self.create_task(self._finish())
        else:
...
class _ServerHostBasedAuth(ServerAuth):
    ...
    async def _start(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Verifying host based auth of user %s " "on host %s with %s host key",
            client_username,
            client_host,
            algorithm,
        )

        if await self._conn.validate_host_based_auth(
            self._username, key_data, client_host, client_username, msg, signature
        ):
            await self.send_success()
        else:
            self.send_failure()
...
class _ServerPublicKeyAuth(ServerAuth):
    ...
    async def _start(self, packet: SSHPacket) -> None:
        ...
        if await self._conn.validate_public_key(
            self._username, key_data, msg, signature
        ):
            if sig_present:
                await self.send_success()
            else:
                self.send_packet(
                    MSG_USERAUTH_PK_OK, String(algorithm), String(key_data)
                )
        else:
            self.send_failure()
...
class _ServerKbdIntAuth(ServerAuth):
    ...
    async def _start(self, packet: SSHPacket) -> None:
        ...
        challenge = await self._conn.get_kbdint_challenge(
            self._username, lang, submethods
        )
    ...
    async def _send_challenge(self, challenge: KbdIntChallenge) -> None:
        ...
        if isinstance(challenge, (tuple, list)):
            ...
            prompts_bytes = (String(prompt) + Boolean(echo) for prompt, echo in prompts)
    ...
    async def _validate_response(self, responses: KbdIntResponse) -> None:
        ...
        next_challenge = await self._conn.validate_kbdint_response(
            self._username, responses
        )
    ...
    _packet_handlers = {MSG_USERAUTH_INFO_RESPONSE: _process_info_response}
    ...
    async def _start(self, packet: SSHPacket) -> None:
        ...
        try:
            if password_change:
                ...
                result = await self._conn.change_password(
                    self._username, password, new_password
                )
            else:
                self.logger.debug1("Trying password auth")

                result = await self._conn.validate_password(self._username, password)
            ...
            self.send_packet(
                MSG_USERAUTH_PASSWD_CHANGEREQ, String(exc.prompt), String(exc.lang)
            )


def register_auth_method(
    alg: bytes, client_handler: Type[ClientAuth], server_handler: Type[ServerAuth]
) -> None:
...
def get_supported_client_auth_methods() -> Sequence[bytes]:
    ...
    return [method for method in _client_auth_handlers if method != b"none"]


def lookup_client_auth(
    conn: "SSHClientConnection", method: bytes
) -> Optional[ClientAuth]:
...
def get_supported_server_auth_methods(conn: "SSHServerConnection") -> Sequence[bytes]:
...
def lookup_server_auth(
    conn: "SSHServerConnection", username: str, method: bytes, packet: SSHPacket
) -> Optional[ServerAuth]:
