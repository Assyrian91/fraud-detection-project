...
def _parse_cd_args(args: bytes) -> Tuple[int, int, bytes]:
    ...
    try:
        ...
        raise _scp_error(SFTPBadMessage, "Invalid copy or dir request") from None
...
async def _parse_path(
    ...
    return (
        cast(Optional["SSHClientConnection"], conn),
        cast(_SCPPath, path),
        close_conn,
    )
...
async def _start_remote(
    ...
    command = (
        b"scp "
        + (b"-f " if source else b"-t ")
        + (b"-d " if must_be_dir else b"")
        + (b"-p " if preserve else b"")
        + (b"-r " if recurse else b"")
        + path
    )

    conn.logger.get_child("sftp").info("Starting remote SCP, args: %s", command[4:])
...
class _SCPHandler:
    ...
    async def await_response(self) -> Optional[Exception]:
        ...
        if result != b"\0":
            ...
            return _scp_error(
                SFTPFailure, reason[:-1], fatal=result != b"\x01", suppress_send=True
            )
    ...
    def send_error(self, exc: Exception) -> None:
        ...
        self.logger.debug1("Sending SCP %serror: %s", "fatal " if fatal else "", reason)

        self._writer.write((b"\x02" if fatal else b"\x01") + b"scp: " + reason + b"\n")
    ...
    async def recv_request(self) -> Tuple[Optional[bytes], Optional[bytes]]:
        ...
        if action not in b"\x01\x02":
            ...
            self.logger.debug1(
                "Received SCP %serror: %s", "fatal " if action != b"\x01" else "", args
            )
    ...
    def handle_error(self, exc: Exception) -> None:
        ...
        if isinstance(exc, BrokenPipeError):
            exc = _scp_error(
                SFTPConnectionLost, "Connection lost", fatal=True, suppress_send=True
            )
...
class _SCPSource(_SCPHandler):
    ...
    async def _make_cd_request(
        self, action: bytes, attrs: SFTPAttrs, size: int, path: bytes
    ) -> None:
        ...
        await self.make_request(action, args.encode("ascii"), self._fs.basename(path))

    async def _make_t_request(self, attrs: SFTPAttrs) -> None:
        """Make an SCP time request"""

        self.logger.info(
            "    Preserving attrs: %s", SFTPAttrs(atime=attrs.atime, mtime=attrs.mtime)
        )
    ...
    async def _send_file(
        self, srcpath: bytes, dstpath: bytes, attrs: SFTPAttrs
    ) -> None:
        ...
        try:
            ...
            while offset < size:
                ...
                if local_exc:
                    ...
                    try:
                        data = cast(bytes, await file_obj.read(blocklen, offset))
    ...
    async def _send_dir(self, srcpath: bytes, dstpath: bytes, attrs: SFTPAttrs) -> None:
    ...
    async def run(self, srcpath: _SCPPath) -> None:
        ...
        try:
            ...
            for name in await SFTPGlob(self._fs).match(srcpath):
                await self._send_files(cast(bytes, name.filename), b"", name.attrs)
...
class _SCPSink(_SCPHandler):
    ...
    async def _recv_file(self, srcpath: bytes, dstpath: bytes, size: int) -> None:
    ...
    async def _recv_dir(self, srcpath: bytes, dstpath: bytes) -> None:
        ...
        if not self._recurse:
            raise _scp_error(SFTPBadMessage, "Directory received without recurse")
        ...
        while True:
            ...
            try:
                if action in b"\x01\x02":
                    raise _scp_error(
                        SFTPFailure, args, fatal=action != b"\x01", suppress_send=True
                    )
                elif action == b"T":
                    if self._preserve:
                    ...
                    try:
                        ...
                        if action == b"D":
                            ...
                            await self._recv_file(new_srcpath, new_dstpath, size)
    ...
    async def run(self, dstpath: _SCPPath) -> None:
        ...
        try:
            ...
            if self._must_be_dir and not await self._fs.isdir(dstpath):
                self.handle_error(_scp_error(SFTPFailure, "Not a directory", dstpath))
            else:
...
class _SCPCopier:
    ...
    async def _forward_response(
        self, src: _SCPHandler, dst: _SCPHandler
    ) -> Optional[Exception]:
            ...
            if action in b"CD":
                try:
                    ...
                    if action == b"C":
                        ...
                        self.logger.info(
                            "  Starting copy of directory %s", b"/".join(pathlist)
                        )
                finally:
                    attrs = SFTPAttrs()
            elif action == b"E":
                if pathlist:
                    self.logger.info(
                        "  Finished copy of directory %s", b"/".join(pathlist)
                    )
...
async def scp(
    srcpaths: Union[_SCPConnPath, Sequence[_SCPConnPath]],
    dstpath: _SCPConnPath = None,
    *,
    preserve: bool = False,
    recurse: bool = False,
    block_size: int = _SCP_BLOCK_SIZE,
...
def run_scp_server(
    sftp_server: MaybeAwait[SFTPServer],
    command: str,
    stdin: "SSHReader[bytes]",
    stdout: "SSHWriter[bytes]",
    stderr: "SSHWriter[bytes]",
) -> MaybeAwait[None]:
