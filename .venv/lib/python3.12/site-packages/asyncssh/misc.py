...
def encode_env(env: Env) -> Iterator[Tuple[bytes, bytes]]:
    ...
    env = cast(
        Sequence[Tuple[BytesOrStr, BytesOrStr]],
...
def get_symbol_names(
    symbols: Mapping[str, int], prefix: str, strip_leading: int = 0
) -> Mapping[int, str]:
    """Return a mapping from values to symbol names for logging"""

    return {
        value: name[strip_leading:]
...
if sys.platform == "win32" and _pywin32_available:  # pragma: no cover
    ...
    def make_sparse_file(file_obj: IO) -> None:
        ...
        win32file.DeviceIoControl(handle, winioctlcon.FSCTL_SET_SPARSE, b"", 0, None)
...
def _parse_units(value: str, suffixes: Mapping[str, int], label: str) -> float:
    ...
    try:
        return sum(
            float(matches[i]) * suffixes[matches[i + 1].lower()]
...
class _ACMWrapper(Generic[_ACM]):
    ...
    async def __aexit__(
        ...
        exit_result = await self._coro_result.__aexit__(exc_type, exc_value, traceback)
...
def set_terminal_size(
    tty: IO, width: int, height: int, pixwidth: int, pixheight: int
) -> None:
    """Set the terminal size of a TTY"""

    fcntl.ioctl(
        tty, termios.TIOCSWINSZ, struct.pack("hhhh", height, width, pixwidth, pixheight)
    )
    ...
    def __init__(self, options: Optional["Options"] = None, **kwargs: object):
        if options:
            if not isinstance(options, type(self)):
                raise TypeError(
                    f"Invalid {type(self).__name__}, " f"got {type(options).__name__}"
                )
...
class Record(metaclass=_RecordMeta):
    ...
    def __str__(self) -> str:
        values = ((k, self._format(k, getattr(self, k))) for k in self.__slots__)
...
class TerminalSizeChanged(Exception):
    ...
    def __init__(self, width: int, height: int, pixwidth: int, pixheight: int):
        super().__init__(
            f"Terminal size change: ({width}, {height}, " f"{pixwidth}, {pixheight})"
        )
        self.width = width
        self.height = height
