...
ProcessSource = Union[
    int, str, socket.socket, PurePath, SSHReader[bytes], asyncio.StreamReader, _File
]

ProcessTarget = Union[
    int, str, socket.socket, PurePath, SSHWriter[bytes], asyncio.StreamWriter, _File
]
...
class _UnicodeReader(_ReaderProtocol, Generic[AnyStr]):
    ...
    def __init__(self, encoding: Optional[str], errors: str, textmode: bool = False):
        super().__init__()
...
class _FileWriter(_UnicodeWriter[AnyStr]):
    ...
    def __init__(
        self, file: IO[bytes], needs_close: bool, encoding: Optional[str], errors: str
    ):
        super().__init__(encoding, errors, hasattr(file, "encoding"))
...
class SSHProcess(SSHStreamSession, Generic[AnyStr]):
    ...
    async def _create_reader(
        ...
        if source == PIPE:
            ...
            reader = _StreamReader(
                self, source, bufsize, datatype, self._encoding, self._errors
            )
        else:
            file: _File
            ...
            if hasattr(file, "read") and (
                inspect.iscoroutinefunction(file.read)
                or inspect.isgeneratorfunction(file.read)
            ):
                reader = _AsyncFileReader(
                    self,
                    cast(_AsyncFileProtocol, file),
                    bufsize,
                    datatype,
                    self._encoding,
                    self._errors,
                ...
                _, protocol = await self._loop.connect_read_pipe(pipe_factory, file)
    ...
    async def _create_writer(
        ...
        if target == PIPE:
            ...
            writer = _StreamWriter(
                self, target, recv_eof, datatype, self._encoding, self._errors
            )
        else:
            file: _File
            needs_close = True
            ...
            if isinstance(target, str):
                ...
                file = os.fdopen(target, "wb", buffering=bufsize, closefd=recv_eof)
            ...
            if hasattr(file, "write") and (
                inspect.iscoroutinefunction(file.write)
                or inspect.isgeneratorfunction(file.write)
            ):
                ...
                writer = _FileWriter(
                    cast(IO[bytes], file), needs_close, self._encoding, self._errors
                )
            else:
                ...
                assert self._loop is not None
                _, protocol = await self._loop.connect_write_pipe(pipe_factory, file)
    ...
    def _should_block_drain(self, datatype: DataType) -> bool:
        ...
        return datatype in self._readers or super()._should_block_drain(datatype)

    def _should_pause_reading(self) -> bool:
        """Return whether to pause reading from the channel"""

        return bool(self._paused_write_streams) or super()._should_pause_reading()
    ...
    def feed_recv_buf(
        self, datatype: DataType, writer: _WriterProtocol[AnyStr]
    ) -> None:
    ...
    def set_reader(
        self, reader: Optional[_ReaderProtocol], send_eof: bool, datatype: DataType
    ) -> None:
    ...
    def set_writer(
        self,
        writer: Optional[_WriterProtocol[AnyStr]],
        recv_eof: bool,
        datatype: DataType,
    ) -> None:
...
class SSHClientProcess(SSHProcess[AnyStr], SSHClientStreamSession[AnyStr]):
    ...
    def feed_exception(self, exc: Exception) -> None:
        ...
        if isinstance(exc, TerminalSizeChanged):
            self._chan.change_terminal_size(
    ...
    async def redirect(
        ...
        if stderr:
            await self._create_writer(
                stderr, bufsize, send_eof, recv_eof, EXTENDED_DATA_STDERR
            )
    ...
    def collect_output(self) -> Tuple[AnyStr, AnyStr]:
        ...
        return (self._collect_output(), self._collect_output(EXTENDED_DATA_STDERR))

    # pylint: disable=redefined-builtin
    async def communicate(
        self, input: Optional[AnyStr] = None
    ) -> Tuple[AnyStr, AnyStr]:
    ...
    async def wait(
        self, check: bool = False, timeout: Optional[float] = None
    ) -> SSHCompletedProcess:
...
class SSHServerProcess(SSHProcess[AnyStr], SSHServerStreamSession[AnyStr]):
    ...
    def __init__(
        self,
        process_factory: SSHServerProcessFactory,
        sftp_factory: Optional[SFTPServerFactory],
        sftp_version: int,
        allow_scp: bool,
    ):
        super().__init__(self._start_process, sftp_factory, sftp_version, allow_scp)
    ...
    def _start_process(
        self,
        stdin: SSHReader[AnyStr],
        stdout: SSHWriter[AnyStr],
        stderr: SSHWriter[AnyStr],
    ) -> MaybeAwait[None]:
    ...
    async def redirect(
        self,
        stdin: Optional[ProcessTarget] = None,
