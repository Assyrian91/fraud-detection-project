...
_signal_numbers = {
    k[3:]: int(v) for (k, v) in vars(_signal).items() if _signal_regex.match(k)
}
...
class SSHChannel(Generic[AnyStr], SSHPacketHandler):
    ...
    def _cleanup(self, exc: Optional[Exception] = None) -> None:
        ...
        if self._session is not None:
            ...
            try:
                ...
                self.logger.debug1(
                    "Uncaught exception in session ignored", exc_info=sys.exc_info
                )
        ...
        if self._conn:  # pragma: no branch
            self.logger.info("Channel closed%s", ": " + str(exc) if exc else "")
    ...
    def _flush_send_buf(self) -> None:
        ...
        while self._send_buf and self._send_window:
            ...
            if datatype is None:
                ...
                self.send_packet(
                    MSG_CHANNEL_EXTENDED_DATA, UInt32(datatype), String(data)
                )
    ...
    def _flush_recv_buf(self, exc: Optional[Exception] = None) -> None:
        ...
        if not self._recv_buf and self._recv_paused != "starting":
            if (
                self._encoding
                and not exc
                and self._recv_state in ("eof_pending", "close_pending")
            ):
                try:
    ...
    def _deliver_data(self, data: bytes, datatype: DataType) -> None:
        ...
        if self._recv_window < self._init_recv_window / 2:
            ...
            self.logger.debug2(
                "Sending window adjust of %d bytes, " "new window %d",
                adjust,
                self._init_recv_window,
            )
    ...
    def process_open(
        ...
        send_window: int,
        send_pktsize: int,
        session: MaybeAwait[SSHSession[AnyStr]],
    ) -> None:
        ...
        self.logger.debug2(
            "  Initial send window %d, packet size %d", send_window, send_pktsize
        )
    ...
    def _wrap_session(
        self, session: SSHSession[AnyStr]
    ...
    async def _finish_open_request(
        self, result: MaybeAwait[SSHSession[AnyStr]]
    ) -> None:
        ...
        try:
            ...
            self.logger.debug2(
                "  Initial recv window %d, packet size %d",
                self._recv_window,
                self._recv_pktsize,
            )
            ...
            self._conn.send_channel_open_confirmation(
                self._send_chan, self._recv_chan, self._recv_window, self._recv_pktsize
            )
            ...
            if self._conn:
                assert self._send_chan is not None
                self._conn.send_channel_open_failure(
                    self._send_chan, exc.code, exc.reason, exc.lang
                )

            self._loop.call_soon(self._cleanup)

    def process_open_confirmation(
        self, send_chan: int, send_window: int, send_pktsize: int, packet: SSHPacket
    ) -> None:
    ...
    def _process_window_adjust(
        ...
        self.logger.debug2(
            "Received window adjust of %d bytes, " "new window %d",
            adjust,
            self._send_window,
        )
    ...
    def _process_data(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
        ...
        self.logger.debug2(
            "Received %d data byte%s", datalen, "s" if datalen > 1 else ""
        )

        self._accept_data(data)

    def _process_extended_data(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
    ...
    def _process_keepalive_at_openssh_dot_com_request(self, packet: SSHPacket) -> bool:
    ...
    def _send_request(
        ...
        self.send_packet(
            MSG_CHANNEL_REQUEST, String(request), Boolean(want_reply), *args
        )

    async def _make_request(self, request: bytes, *args: bytes) -> Optional[bool]:
    ...
    def get_extra_info(self, name: str, default: Any = None) -> Any:
        ...
        return self._extra.get(
            name, self._conn.get_extra_info(name, default) if self._conn else default
        )
    ...
    def set_write_buffer_limits(
        ...
        self.logger.debug1(
            "Set write buffer limits: low-water=%d, " "high-water=%d", low, high
        )
    ...
    def write(self, data: AnyStr, datatype: DataType = None) -> None:
        ...
        self.logger.debug2(
            "Sending %d data byte%s%s", datalen, "s" if datalen > 1 else "", typename
        )
    ...
    def writelines(
        self, list_of_data: Iterable[AnyStr], datatype: DataType = None
    ) -> None:
...
class SSHClientChannel(SSHChannel, Generic[AnyStr]):
    ...
    async def create(
        self,
        session_factory: SSHClientSessionFactory[AnyStr],
        ...
        if request_pty:
            ...
            if not term_size:
                ...
                self.logger.debug1(
                    "  Terminal size: %sx%s (%sx%s pixels)",
                    width,
                    height,
                    pixwidth,
                    pixheight,
                ...
                raise ValueError(
                    "If set, terminal size must be a tuple of " "2 or 4 integers"
                )
            ...
            if not (
                ...
                self.close()
                raise ChannelOpenError(OPEN_REQUEST_PTY_FAILED, "PTY request failed")

        if x11_forwarding:
            self.logger.debug1("  X11 forwarding enabled")

            try:
                attach_result: Optional[
                ...
                if x11_forwarding != "ignore_failure":
                    raise ChannelOpenError(
                        OPEN_REQUEST_X11_FORWARDING_FAILED, str(exc)
                    ) from None
                else:
                    attach_result = None
            ...
            if attach_result:
                ...
                result = await self._make_request(
                    b"x11-req",
                    Boolean(x11_single_connection),
                    String(auth_proto),
                    String(binascii.b2a_hex(remote_auth)),
                    UInt32(screen),
                )
                ...
                if not result:
                    ...
                    if x11_forwarding != "ignore_failure":
                        ...
                        self.logger.info("  X11 forwarding request failure ignored")
        ...
        if not result:
            self.close()
            raise ChannelOpenError(
                OPEN_REQUEST_SESSION_FAILED, "Session request failed"
            )
    ...
    def _process_xon_xoff_request(self, packet: SSHPacket) -> bool:
        ...
        self.logger.info(
            "Received XON/XOFF flow control %s request",
    ...
    def change_terminal_size(
        self, width: int, height: int, pixwidth: int = 0, pixheight: int = 0
    ) -> None:
        ...
        if pixwidth or pixheight:
            self.logger.info(
                "Sending window size change: %sx%s (%sx%s pixels)",
                width,
                height,
                pixwidth,
                pixheight,
...
class SSHServerChannel(SSHChannel, Generic[AnyStr]):
    ...
    def _wrap_session(
        ...
        if self._line_editor:
            ...
            editor_chan = SSHLineEditorChannel(
                server_chan,
                server_session,
                self._line_echo,
                self._line_history,
    ...
    def _process_pty_req_request(self, packet: SSHPacket) -> bool:
        ...
        if (
            not self._allow_pty
            or not self._conn.check_key_permission("pty")
            or not self._conn.check_certificate_permission("pty")
        ):
    ...
    def _process_x11_req_request(self, packet: SSHPacket) -> Optional[bool]:
        ...
        self._conn.create_task(
            self._finish_x11_req_request(auth_proto, auth_data, screen), self.logger
        )
        return None

    async def _finish_x11_req_request(
        self, auth_proto: bytes, auth_data: bytes, screen: int
    ) -> None:
    ...
    def _start_session(
        self, command: Optional[str] = None, subsystem: Optional[str] = None
    ) -> bool:
        """Tell the session what type of channel is being requested"""

        forced_command = cast(str, self._conn.get_certificate_option("force-command"))
    ...
    def _process_window_change_request(self, packet: SSHPacket) -> bool:
        ...
        if pixwidth or pixheight:
            self.logger.info(
                "Received window change: %sx%s (%sx%s pixels)",
                width,
                height,
                pixwidth,
                pixheight,
    ...
    def set_xon_xoff(self, client_can_do: bool) -> None:
        ...
        self.logger.info(
            "Sending XON/XOFF flow control %s", "enable" if client_can_do else "disable"
        )
...
class SSHForwardChannel(SSHChannel, Generic[AnyStr]):
    ...
    async def _open_forward(
        self, session_factory: SSHSessionFactory[AnyStr], chantype: bytes, *args: bytes
    ) -> SSHSession[AnyStr]:
...
class SSHTCPChannel(SSHForwardChannel, Generic[AnyStr]):
    ...
    async def connect(
        ...
        return await self._open_tcp(
            session_factory, b"direct-tcpip", host, port, orig_host, orig_port
        )
    ...
    async def accept(
        ...
        return await self._open_tcp(
            session_factory, b"forwarded-tcpip", host, port, orig_host, orig_port
        )

    def set_inbound_peer_names(
...
class SSHUNIXChannel(SSHForwardChannel, Generic[AnyStr]):
    ...
    async def _open_unix(
        ...
        *args: bytes,
        ...
        return cast(
            SSHUNIXSession[AnyStr],
            await self._open_forward(session_factory, chantype, String(path), *args),
        )

    async def connect(
        self, session_factory: SSHUNIXSessionFactory[AnyStr], path: str
    ) -> SSHUNIXSession[AnyStr]:
    ...
    async def accept(
        ...
        return await self._open_unix(
            session_factory, b"forwarded-streamlocal@openssh.com", path, String("")
        )
...
class SSHTunTapChannel(SSHForwardChannel[bytes]):
    ...
    async def open(
        self, session_factory: SSHTunTapSessionFactory, mode: int, unit: Optional[int]
    ) -> SSHTunTapSession:
        ...
        return cast(
            SSHTunTapSession,
            await self._open_forward(
                session_factory, b"tun@openssh.com", UInt32(mode), UInt32(unit)
            ),
        )
...
class SSHX11Channel(SSHForwardChannel[bytes]):
    ...
    async def open(
        ...
        self.set_extra_info(
            local_peername=(orig_host, orig_port), remote_peername=("", 0)
        )

        return cast(
            SSHTCPSession[bytes],
            await self._open_forward(
                session_factory, b"x11", String(orig_host), UInt32(orig_port)
            ),
        )
    ...
    def set_inbound_peer_names(self, orig_host: str, orig_port: int) -> None:
        ...
        self.set_extra_info(
            local_peername=("", 0), remote_peername=(orig_host, orig_port)
        )
...
class SSHAgentChannel(SSHForwardChannel[bytes]):
    ...
    async def open(
        ...
        return cast(
            SSHUNIXSession[bytes],
            await self._open_forward(session_factory, b"auth-agent@openssh.com"),
        )
