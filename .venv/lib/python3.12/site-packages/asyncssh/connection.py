...
_ServerHostKeysHandler = Optional[
    Callable[[List[SSHKey], List[SSHKey], List[SSHKey], List[SSHKey]], MaybeAwait[None]]
]
...
class _TunnelListenerProtocol(_TunnelProtocol, Protocol):
    ...
    async def create_server(
        self, session_factory: TCPListenerFactory, listen_host: str, listen_port: int
    ) -> SSHListener:
        """Create an inbound tunnel listener"""


_AcceptHandler = Optional[Callable[["SSHConnection"], MaybeAwait[None]]]
_ErrorHandler = Optional[Callable[["SSHConnection", Optional[Exception]], None]]
...
async def _canonicalize_host(
    loop: asyncio.AbstractEventLoop, options: "SSHConnectionOptions"
) -> Optional[str]:
    ...
    for domain in options.canonical_domains:
        ...
        try:
            addrinfo = await loop.getaddrinfo(canon_host, 0, flags=socket.AI_CANONNAME)
        ...
        if cname and cname != canon_host:
            logger.debug1(
                "  Checking CNAME rules for hostname %s " "with CNAME %s",
                canon_host,
                cname,
            )
            ...
            for patterns in options.canonicalize_permitted_cnames:
                ...
                if host_pat.matches(canon_host) and cname_pat.matches(cname):
                    logger.info(
                        "Hostname canonicalization to CNAME " "applied: %s -> %s",
                        options.host,
                        cname,
                    )
                    return cname
        ...
        logger.info(
            "Hostname canonicalization applied: %s -> %s", options.host, canon_host
        )
...
async def _open_proxy(
    ...
    class _ProxyCommandTunnel(asyncio.SubprocessProtocol):
        ...
        def connection_made(self, transport: asyncio.BaseTransport) -> None:
            ...
            self._stdin = cast(
                asyncio.WriteTransport, self._transport.get_pipe_transport(0)
            )
...
async def _open_tunnel(
    tunnels: object, options: _Options, config: DefTuple[ConfigPaths]
...
async def _listen(
    ...
    if sock:
        ...
        # pylint: disable=broad-except
        try:
            tunnel_server = await new_tunnel.create_server(tunnel_factory, host, port)
        except Exception:
            new_tunnel.close()
            await new_tunnel.wait_closed()
            raise
...
def _expand_algs(
    ...
    for pat in algs.split(","):
        ...
        matches = [alg for alg in possible_algs if pattern.matches(alg.decode("ascii"))]
...
def _select_algs(
    ...
    if algs in ((), "default"):
        ...
        if isinstance(algs, str):
            expanded_algs = _expand_algs(
                alg_type, algs, possible_algs, default_algs, strict_match
            )
        else:
        ...
        for alg in expanded_algs:
            if alg not in possible_algs:
                raise ValueError(
                    f'{alg.decode("ascii")} is not a valid ' f"{alg_type} algorithm"
                )
...
def _select_host_key_algs(
    algs: _AlgsArg, config_algs: _AlgsArg, default_algs: List[bytes]
) -> Sequence[bytes]:
    """Select a set of allowed host key algorithms"""

    possible_algs = (
        get_x509_certificate_algs() + get_certificate_algs() + get_public_key_algs()
    )

    return _select_algs("host key", algs, config_algs, possible_algs, default_algs)


def _validate_algs(
    config: SSHConfig,
    kex_algs_arg: _AlgsArg,
    enc_algs_arg: _AlgsArg,
    mac_algs_arg: _AlgsArg,
    cmp_algs_arg: _AlgsArg,
    sig_algs_arg: _AlgsArg,
    allow_x509: bool,
) -> Tuple[
    Sequence[bytes], Sequence[bytes], Sequence[bytes], Sequence[bytes], Sequence[bytes]
]:
    """Validate requested algorithms"""

    kex_algs = _select_algs(
        "key exchange",
        kex_algs_arg,
    ...
    sig_algs = _select_algs(
        "signature",
        sig_algs_arg,
        cast(_AlgsArg, config.get("CASignatureAlgorithms", ())),
        allowed_sig_algs,
        default_sig_algs,
    )
...
class SSHAcceptor:
    ...
    def __init__(self, server: asyncio.AbstractServer, options: "SSHConnectionOptions"):
        self._server = server
...
class SSHConnection(SSHPacketHandler, asyncio.Protocol):
    ...
    def __init__(
        ...
        self._global_request_waiters: "List[asyncio.Future[_GlobalRequestResult]]" = []

        self._local_listeners: Dict[ListenKey, SSHListener] = {}

        self._x11_listener: Union[
            None, SSHX11ClientListener, SSHX11ServerListener
        ] = None
        ...
        self._logger = logger.get_child(context=f"conn={self._get_next_conn()}")
    ...
    def _cleanup(self, exc: Optional[Exception]) -> None:
        ...
        while self._global_request_waiters:
            self._process_global_response(MSG_REQUEST_FAILURE, 0, SSHPacket(b""))
        ...
        if self._owner:  # pragma: no branch
            ...
            try:
                ...
                self.logger.debug1(
                    "Uncaught exception in owner ignored", exc_info=sys.exc_info
                )
    ...
    def _reap_task(
        self, task_logger: Optional[SSHLogger], task: "asyncio.Task[None]"
    ) -> None:
    ...
    def create_task(
        self, coro: Awaitable[None], task_logger: Optional[SSHLogger] = None
    ) -> "asyncio.Task[None]":
    ...
    def _match_known_hosts(
        self, known_hosts: KnownHostsArg, host: str, addr: str, port: Optional[int]
    ) -> None:
    ...
    def _validate_openssh_host_certificate(
        self, host: str, addr: str, port: int, cert: SSHOpenSSHCertificate
    ) -> SSHKey:
        ...
        if self._trusted_ca_keys is not None:
            ...
            if (
                ...
                and not self._owner.validate_host_ca_key(
                    host, addr, port, cert.signing_key
                )
            ):
    ...
    def _validate_x509_host_certificate_chain(
        self, host: str, cert: SSHX509CertificateChain
    ) -> SSHKey:
        """Validate an X.509 host certificate"""

        if self._x509_revoked_subjects and any(
            pattern.matches(cert.subject) for pattern in self._x509_revoked_subjects
        ):
            raise ValueError("X.509 subject name is revoked")

        if self._x509_trusted_subjects and not any(
            pattern.matches(cert.subject) for pattern in self._x509_trusted_subjects
        ):
    ...
    def _validate_host_key(
        self, host: str, addr: str, port: int, key_data: bytes
    ) -> SSHKey:
        ...
        try:
            ...
            if self._trusted_host_keys is not None:
                ...
                if (
                    ...
                    and not self._owner.validate_host_public_key(host, addr, port, key)
                ):
    ...
    def connection_made(self, transport: asyncio.BaseTransport) -> None:
        ...
        if sock:
            sock.setsockopt(
                socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1 if self._tcp_keepalive else 0
            )
    ...
    def internal_error(
        ...
        error_logger: Optional[SSHLogger] = None,
    ) -> None:
    ...
    def add_channel(self, chan: SSHChannel[AnyStr]) -> int:
        ...
        if not self._transport:
            raise ChannelOpenError(OPEN_CONNECT_FAILED, "SSH connection closed")
    ...
    def _choose_alg(
        self, alg_type: str, local_algs: Sequence[bytes], remote_algs: Sequence[bytes]
    ) -> bytes:
    ...
    def _recv_pkthdr(self) -> bool:
        ...
        if self._recv_encryption:
            self._packet, pktlen = self._recv_encryption.decrypt_header(
                self._recv_seq, self._packet, 4
            )
        else:
    ...
    def _recv_packet(self) -> bool:
        ...
        if self._decompressor and (
            self._auth_complete or not self._decompress_after_auth
        ):
        ...
        if MSG_KEX_FIRST <= pkttype <= MSG_KEX_LAST:
            ...
            self._strict_kex
            and not self._recv_encryption
            and MSG_IGNORE <= pkttype <= MSG_DEBUG
        ):
    ...
    def send_packet(
        self, pkttype: int, *args: bytes, handler: Optional[SSHPacketLogger] = None
    ) -> None:
        """Send an SSH packet"""

        if (
            self._auth_complete
            and self._kex_complete
            and (
                self._rekey_bytes_sent >= self._rekey_bytes
                or (self._rekey_seconds and time.monotonic() >= self._rekey_time)
            )
        ):
            self._send_kexinit()
            self._kexinit_sent = True
        ...
        if (
            (
                (
                    pkttype in {MSG_DEBUG, MSG_SERVICE_REQUEST, MSG_SERVICE_ACCEPT}
                    or pkttype > MSG_KEX_LAST
                )
                and not self._kex_complete
            )
            or (
                pkttype == MSG_USERAUTH_BANNER
                and not (self._auth_in_progress or self._auth_complete)
            )
            or (pkttype > MSG_USERAUTH_LAST and not self._auth_complete)
        ):
        ...
        if self._compressor and (self._auth_complete or not self._compress_after_auth):
    ...
    def _send_disconnect(self, code: int, reason: str, lang: str) -> None:
        ...
        self.send_packet(MSG_DISCONNECT, UInt32(code), String(reason), String(lang))
    ...
    def _send_kexinit(self) -> None:
        ...
        kex_algs = (
            expand_kex_algs(self._kex_algs, gss_mechs, bool(self._server_host_key_algs))
    ...
    def send_newkeys(self, k: bytes, h: bytes) -> None:
        ...
        iv_cs = self._kex.compute_key(k, h, b"A", self._session_id, enc_ivsize_cs)
        iv_sc = self._kex.compute_key(k, h, b"B", self._session_id, enc_ivsize_sc)
        enc_key_cs = self._kex.compute_key(k, h, b"C", self._session_id, enc_keysize_cs)
        enc_key_sc = self._kex.compute_key(k, h, b"D", self._session_id, enc_keysize_sc)
        mac_key_cs = self._kex.compute_key(k, h, b"E", self._session_id, mac_keysize_cs)
        mac_key_sc = self._kex.compute_key(k, h, b"F", self._session_id, mac_keysize_sc)
        self._kex = None

        next_enc_cs = get_encryption(
            self._enc_alg_cs, enc_key_cs, iv_cs, self._mac_alg_cs, mac_key_cs, etm_cs
        )
        next_enc_sc = get_encryption(
            self._enc_alg_sc, enc_key_sc, iv_sc, self._mac_alg_sc, mac_key_sc, etm_sc
        )
        ...
        if self.is_client():
            ...
            self._extensions_to_send[b"server-sig-algs"] = b",".join(self._sig_algs)
    ...
    def _get_userauth_request_packet(
        self, method: bytes, args: Tuple[bytes, ...]
    ) -> bytes:
        """Get packet data for a user authentication request"""

        return b"".join(
            (
                Byte(MSG_USERAUTH_REQUEST),
                String(self._username),
                String(_CONNECTION_SERVICE),
                String(method),
            )
            + args
        )
    ...
    def get_userauth_request_data(self, method: bytes, *args: bytes) -> bytes:
        ...
        return String(self._session_id) + self._get_userauth_request_packet(
            method, args
        )

    def send_userauth_packet(
        self,
        pkttype: int,
        *args: bytes,
    ...
    async def send_userauth_request(
        self,
        method: bytes,
        *args: bytes,
        ...
        if key:
            ...
            sign_async: Optional[Callable[[bytes], Awaitable[bytes]]] = getattr(
                key, "sign_async", None
            )
        ...
        self.send_userauth_packet(MSG_USERAUTH_REQUEST, packet[1:], trivial=trivial)

    def send_userauth_failure(self, partial_success: bool) -> None:
        """Send a user authentication failure response"""

        methods = get_supported_server_auth_methods(cast(SSHServerConnection, self))

        self.logger.debug2("Remaining auth methods: %s", methods or "None")

        self._auth = None
        self.send_packet(
            MSG_USERAUTH_FAILURE, NameList(methods), Boolean(partial_success)
        )
    ...
    async def send_userauth_success(self) -> None:
        ...
        if self._wait == "auth" and self._waiter and not self._waiter.cancelled():
            self._waiter.set_result(None)
            self._wait = None
            return
    ...
    def send_channel_open_failure(
        self, send_chan: int, code: int, reason: str, lang: str
    ) -> None:
    ...
    def _send_global_request(
        self, request: bytes, *args: bytes, want_reply: bool = False
    ) -> None:
        """Send a global request"""

        self.send_packet(
            MSG_GLOBAL_REQUEST, String(request), Boolean(want_reply), *args
        )

    async def _make_global_request(
        self, request: bytes, *args: bytes
    ) -> Tuple[int, SSHPacket]:
        ...
        waiter: "asyncio.Future[_GlobalRequestResult]" = self._loop.create_future()
    ...
    def _process_debug(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
    ...
    def _process_ext_info(self, _pkttype: int, _pktid: int, packet: SSHPacket) -> None:
        ...
        if self.is_client():
            self._server_sig_algs = set(
                extensions.get(b"server-sig-algs", b"").split(b",")
            )

    async def _process_kexinit(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
        ...
        if self._strict_kex and not self._recv_encryption and self._recv_seq != 0:
            raise ProtocolError(
        ...
        kex_algs = expand_kex_algs(
            self._kex_algs, gss_mechs, bool(self._server_host_key_algs)
        )
        ...
        self._ignore_first_kex = (
            first_kex_follows and self._kex.algorithm != peer_kex_algs[0]
        )
        ...
        if self.is_server():
            ...
            if not cast(SSHServerConnection, self).choose_server_host_key(
                peer_host_key_algs
            ) and not kex_alg.startswith(b"gss-"):
                raise KeyExchangeFailed("Unable to find compatible " "server host key")

        self._enc_alg_cs = self._choose_alg("encryption", self._enc_algs, enc_algs_cs)
        self._enc_alg_sc = self._choose_alg("encryption", self._enc_algs, enc_algs_sc)
        ...
        self._cmp_alg_cs = self._choose_alg("compression", self._cmp_algs, cmp_algs_cs)
        self._cmp_alg_sc = self._choose_alg("compression", self._cmp_algs, cmp_algs_sc)
    ...
    async def _finish_userauth(
        self, begin_auth: bool, method: bytes, packet: SSHPacket
    ) -> None:
        ...
        self._auth = lookup_server_auth(
            cast(SSHServerConnection, self), self._username, method, packet
        )

    def _process_userauth_failure(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
        ...
        self.logger.debug2("Remaining auth methods: %s", auth_methods or "None")

        if (
            self._wait == "auth_methods"
            and self._waiter
            and not self._waiter.cancelled()
        ):
            self._waiter.set_result(None)
        ...
        if self._preferred_auth:
            self.logger.debug2(
                "Preferred auth methods: %s", self._preferred_auth or "None"
            )

            auth_methods = [
                method for method in self._preferred_auth if method in auth_methods
            ]
    ...
    def _process_userauth_banner(
        ...
        if (
            b"cryptlib" in self._server_version
            and packet.get_remaining_payload() == b"\0"
        ):  # pragma: no cover
            packet.get_byte()
    ...
    def _process_global_response(
        self, pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
    ...
    def _process_channel_open(
        ...
        try:
            ...
            if callable(handler):
                ...
                chan.process_open(send_chan, send_window, send_pktsize, session)
            else:
            ...
            self.logger.debug1(
                "Open failed for channel type %s: %s", chantype, exc.reason
            )

            self.send_channel_open_failure(send_chan, exc.code, exc.reason, exc.lang)

    def _process_channel_open_confirmation(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
        ...
        if chan:
            chan.process_open_confirmation(send_chan, send_window, send_pktsize, packet)
        else:
            self.logger.debug1(
                "Received open confirmation for unknown " "channel %d", recv_chan
            )

            raise ProtocolError("Invalid channel number")

    def _process_channel_open_failure(
        self, _pkttype: int, _pktid: int, packet: SSHPacket
    ) -> None:
        ...
        if chan:
            ...
            self.logger.debug1(
                "Received open failure for unknown " "channel %d", recv_chan
            )
    ...
    def set_keepalive(
        self, interval: Union[None, float, str] = None, count_max: Optional[int] = None
    ) -> None:
    ...
    def send_debug(
        self, msg: str, lang: str = DEFAULT_LANG, always_display: bool = False
    ) -> None:
        ...
        self.logger.debug1(
            "Sending debug message: %s%s",
            msg,
            " (always display)" if always_display else "",
        )

        self.send_packet(MSG_DEBUG, Boolean(always_display), String(msg), String(lang))
    ...
    def create_tcp_channel(
        ...
        return SSHTCPChannel(self, self._loop, encoding, errors, window, max_pktsize)

    def create_unix_channel(
        self,
        encoding: Optional[str] = None,
        errors: str = "strict",
        window: int = _DEFAULT_WINDOW,
        ...
        return SSHUNIXChannel(self, self._loop, encoding, errors, window, max_pktsize)

    def create_tuntap_channel(
        ...
        return SSHTunTapChannel(self, self._loop, None, "strict", window, max_pktsize)
    ...
    def create_x11_channel(
        ...
        return SSHX11Channel(self, self._loop, None, "strict", window, max_pktsize)
    ...
    def create_agent_channel(
        ...
        return SSHAgentChannel(self, self._loop, None, "strict", window, max_pktsize)

    async def create_connection(
        self,
        session_factory: SSHTCPSessionFactory[AnyStr],
        remote_host: str,
        remote_port: int,
        orig_host: str = "",
        orig_port: int = 0,
    ...
    async def forward_connection(self, dest_host: str, dest_port: int) -> SSHForwarder:
        ...
        try:
            _, peer = await self._loop.create_connection(
                SSHForwarder, dest_host, dest_port
            )

            self.logger.info(
                "  Forwarding TCP connection to %s", (dest_host, dest_port)
            )
    ...
    async def forward_unix_connection(self, dest_path: str) -> SSHForwarder:
        ...
        try:
            _, peer = await self._loop.create_unix_connection(SSHForwarder, dest_path)
    ...
    async def forward_local_port(
        ...
        async def tunnel_connection(
            session_factory: SSHTCPSessionFactory[bytes], orig_host: str, orig_port: int
        ) -> Tuple[SSHTCPChannel[bytes], SSHTCPSession[bytes]]:
            ...
            if accept_handler:
                ...
                if not result:
                    self.logger.info(
                        "Request for TCP forwarding from "
                    ...
                    raise ChannelOpenError(
                        OPEN_ADMINISTRATIVELY_PROHIBITED, "Connection forwarding denied"
                    )

            return await self.create_connection(
                session_factory, dest_host, dest_port, orig_host, orig_port
            )

        if (listen_host, listen_port) == (dest_host, dest_port):
            self.logger.info(
                "Creating local TCP forwarder on %s", (listen_host, listen_port)
            )
        else:
            self.logger.info(
                "Creating local TCP forwarder from %s to %s",
    ...
    @async_context_manager
    async def forward_local_path(self, listen_path: str, dest_path: str) -> SSHListener:
        ...
        self.logger.info(
            "Creating local UNIX forwarder from %s to %s", listen_path, dest_path
        )
    ...
    def forward_tuntap(self, mode: int, unit: Optional[int]) -> SSHForwarder:
        ...
        try:
            ...
            self.logger.info(
                "  Forwarding layer %d traffic to %s",
        ...
        return SSHForwarder(cast(SSHForwarder, peer), extra={"interface": interface})
...
class SSHClientConnection(SSHConnection):
    ...
    def __init__(
        ...
        super().__init__(loop, options, acceptor, error_handler, wait, server=False)
        ...
        if options.preferred_auth != ():
            self._preferred_auth = [
                method.encode("ascii") for method in options.preferred_auth
            ]
        else:
        ...
        if gss_host:
            try:
                self._gss = GSSClient(
                    gss_host, options.gss_store, options.gss_delegate_creds
                )
        ...
        self._remote_listeners: Dict[
            ListenKey, Union[SSHTCPClientListener, SSHUNIXClientListener]
        ] = {}
    ...
    def _connection_made(self) -> None:
        ...
        if self._options.client_host_keysign:
            sock = cast(socket.socket, self._transport.get_extra_info("socket"))

            self._client_host_keys = list(
                get_keysign_keys(
                    self._options.client_host_keysign,
                    sock.fileno(),
        ...
        if self._known_hosts is None:
            ...
            if not self._known_hosts:
                ...
                if default_known_hosts.is_file() and os.access(
                    default_known_hosts, os.R_OK
                ):
            ...
            self._match_known_hosts(
                cast(KnownHostsArg, self._known_hosts),
        ...
        self._server_host_key_algs = _select_host_key_algs(
            ...
            cast(DefTuple[str], self._options.config.get("HostKeyAlgorithms", ())),
            default_host_key_algs,
        )

        self.logger.info("Connected to SSH server at %s", (self._host, self._port))

        if self._options.proxy_command:
            proxy_command = " ".join(
                shlex.quote(arg) for arg in self._options.proxy_command
            )
            self.logger.info("  Proxy command: %s", proxy_command)
        else:
            self.logger.info(
                "  Local address: %s", (self._local_addr, self._local_port)
            )
            self.logger.info("  Peer address: %s", (self._peer_addr, self._peer_port))
    ...
    async def host_based_auth_requested(
        self,
        ...
        while True:
            ...
            if self._choose_signature_alg(key):
                if (
                    key.algorithm == b"ssh-rsa-cert-v01@openssh.com"
                    and key.sig_algorithm != b"ssh-rsa"
                ):
    ...
    async def kbdint_challenge_received(
        ...
        if self._kbdint_password_auth:
            ...
            result = self._owner.kbdint_challenge_received(
                name, instructions, lang, prompts
            )
    ...
    def _process_session_open(
        ...
        raise ChannelOpenError(
            OPEN_ADMINISTRATIVELY_PROHIBITED, "Session open forbidden on client"
        )

    def _process_direct_tcpip_open(
        self, _packet: SSHPacket
    ) -> Tuple[SSHTCPChannel[bytes], SSHTCPSession[bytes]]:
        ...
        raise ChannelOpenError(
            OPEN_ADMINISTRATIVELY_PROHIBITED, "Direct TCP/IP open forbidden on client"
        )

    def _process_forwarded_tcpip_open(
        self, packet: SSHPacket
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid forwarded TCP/IP channel " "open request"
            ) from None
        ...
        listener = cast(
            SSHTCPClientListener[bytes],
            self._remote_listeners.get((dest_host, dest_port))
            or self._dynamic_remote_listeners.get(dest_host),
        )
        ...
        if listener:
            ...
            self.logger.info(
                "Accepted forwarded TCP connection on %s", (dest_host, dest_port)
            )
    ...
    async def close_client_tcp_listener(
        ...
        self.logger.info("Closed remote TCP listener on %s", (listen_host, listen_port))
    ...
    def _process_direct_streamlocal_at_openssh_dot_com_open(
        ...
        raise ChannelOpenError(
            ...
            "Direct UNIX domain socket open " "forbidden on client",
        )
    ...
    def _process_tun_at_openssh_dot_com_open(
        ...
        raise ChannelOpenError(
            OPEN_ADMINISTRATIVELY_PROHIBITED, "TUN/TAP request forbidden on client"
        )
    ...
    def _process_forwarded_streamlocal_at_openssh_dot_com_open(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid forwarded UNIX domain channel " "open request"
            ) from None

        listener = cast(
            SSHUNIXClientListener[bytes], self._remote_listeners.get(dest_path)
        )
    ...
    def _process_x11_open(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid forwarded X11 channel " "open request"
            ) from None
        ...
        if self._x11_listener:
            ...
            raise ChannelOpenError(OPEN_CONNECT_FAILED, "X11 forwarding disabled")
    ...
    def _process_auth_agent_at_openssh_dot_com_open(
        ...
        if self._agent_forward_path:
            ...
            raise ChannelOpenError(
                OPEN_CONNECT_FAILED, "Auth agent forwarding disabled"
            )
    ...
    async def _finish_hostkeys(self, packet: SSHPacket) -> None:
        ...
        if prove:
            ...
            if pkttype == MSG_REQUEST_SUCCESS:
                prefix = String("hostkeys-prove-00@openssh.com") + String(
                    self._session_id
                )
        ...
        self.logger.info(
            f"Server host key report: {len(added)} added, "
            f"{len(removed)} removed, {len(retained)} retained, "
            f"{len(revoked)} revoked"
        )

        result = self._server_host_keys_handler(added, removed, retained, revoked)
    ...
    async def attach_x11_listener(
        self,
        chan: SSHClientChannel[AnyStr],
    ...
    async def create_session(
        self,
        session_factory: SSHClientSessionFactory,
        ...
        *,
        ...
        chan = SSHClientChannel(self, self._loop, encoding, errors, window, max_pktsize)

        session = await chan.create(
            session_factory,
            command,
            subsystem,
            new_env,
    ...
    async def open_session(
        ...
        chan, session = await self.create_session(
            SSHClientStreamSession, *args, **kwargs
        )  # type: ignore
    ...
    async def create_process(
        ...
        **kwargs: object,
        ...
        await process.redirect(new_stdin, stdout, stderr, bufsize, send_eof, recv_eof)

        return process

    async def create_subprocess(
        self,
        protocol_factory: SubprocessFactory,
        command: DefTuple[Optional[str]] = (),
        bufsize: int = io.DEFAULT_BUFFER_SIZE,
        input: Optional[AnyStr] = None,
        stdin: ProcessSource = PIPE,
        ...
        **kwargs: object,
        ...
        _, transport = await self.create_session(
            transport_factory, command, encoding=encoding, **kwargs
        )  # type: ignore
        ...
        if input:
            stdin_pipe = cast(SSHSubprocessWritePipe, transport.get_pipe_transport(0))
    ...
    async def create_connection(
        ...
        *,
        ...
        self.logger.info(
            "Opening direct TCP connection to %s", (remote_host, remote_port)
        )
        self.logger.info("  Client address: %s", (orig_host, orig_port))

        chan = self.create_tcp_channel(encoding, errors, window, max_pktsize)

        session = await chan.connect(
            session_factory, remote_host, remote_port, orig_host, orig_port
        )
    ...
    async def open_connection(
        ...
        chan, session = await self.create_connection(
            SSHTCPStreamSession, *args, **kwargs
        )  # type: ignore
    ...
    async def create_server(
        ...
        *,
        ...
        self.logger.info(
            "Creating remote TCP listener on %s", (listen_host, listen_port)
        )
        ...
        if pkttype == MSG_REQUEST_SUCCESS:
            ...
            listener = SSHTCPClientListener[AnyStr](
                self,
                session_factory,
                listen_host,
                listen_port,
    ...
    async def start_server(
        self, handler_factory: _TCPServerHandlerFactory, *args: object, **kwargs: object
    ) -> SSHListener:
    ...
    async def open_unix_connection(
        ...
        chan, session = await self.create_unix_connection(
            SSHUNIXStreamSession, *args, **kwargs
        )  # type: ignore
    ...
    async def create_unix_server(
        ...
        *,
        ...
        if pkttype == MSG_REQUEST_SUCCESS:
            listener = SSHUNIXClientListener[AnyStr](
                self,
                session_factory,
                listen_path,
                encoding,
    ...
    async def start_unix_server(
        self,
        handler_factory: _UNIXServerHandlerFactory,
        *args: object,
        **kwargs: object,
    ) -> SSHListener:
    ...
    async def create_ssh_connection(
        ...
        **kwargs: object,
        ...
        return await create_connection(
            client_factory, host, port, tunnel=self, **kwargs
        )  # type: ignore

    @async_context_manager
    async def connect_ssh(
        self, host: str, port: DefTuple[int] = (), **kwargs: object
    ) -> "SSHClientConnection":
    ...
    async def connect_reverse_ssh(
        ...
        return await connect_reverse(host, port, tunnel=self, **kwargs)  # type: ignore

    @async_context_manager
    async def listen_ssh(
        self, host: str = "", port: DefTuple[int] = (), **kwargs: object
    ) -> SSHAcceptor:
    ...
    async def listen_reverse_ssh(
        ...
        return await listen_reverse(host, port, tunnel=self, **kwargs)  # type: ignore

    async def create_tun(
        self,
        session_factory: SSHTunTapSessionFactory,
        ...
        *,
        ...
        self.logger.info(
            "Opening layer 3 tunnel to remote unit %s",
            "any" if remote_unit is None else str(remote_unit),
        )

        chan = self.create_tuntap_channel(window, max_pktsize)

        session = await chan.open(
            session_factory, SSH_TUN_MODE_POINTTOPOINT, remote_unit
        )
    ...
    async def create_tap(
        ...
        *,
        ...
        self.logger.info(
            "Opening layer 2 tunnel to remote unit %s",
            "any" if remote_unit is None else str(remote_unit),
        )

        chan = self.create_tuntap_channel(window, max_pktsize)

        session = await chan.open(session_factory, SSH_TUN_MODE_ETHERNET, remote_unit)
    ...
    async def open_tun(
        ...
        chan, session = await self.create_tun(
            SSHTunTapStreamSession, *args, **kwargs
        )  # type: ignore
    ...
    async def open_tap(
        ...
        chan, session = await self.create_tap(
            SSHTunTapStreamSession, *args, **kwargs
        )  # type: ignore
    ...
    async def forward_local_path_to_port(
        ...
        async def tunnel_connection(
            ...
            return await self.create_connection(
                session_factory, dest_host, dest_port, "", 0
            )

        self.logger.info(
            "Creating local UNIX forwarder from %s to %s",
            listen_path,
            (dest_host, dest_port),
        )
    ...
    async def forward_remote_port(
        self, listen_host: str, listen_port: int, dest_host: str, dest_port: int
    ) -> SSHListener:
        ...
        def session_factory(
            ...
            return cast(
                Awaitable[SSHTCPSession], self.forward_connection(dest_host, dest_port)
            )

        self.logger.info(
            "Creating remote TCP forwarder from %s to %s",
            (listen_host, listen_port),
            (dest_host, dest_port),
        )

        return await self.create_server(session_factory, listen_host, listen_port)
    ...
    async def forward_remote_path(
        ...
        def session_factory() -> Awaitable[SSHUNIXSession[bytes]]:
            ...
            return cast(
                Awaitable[SSHUNIXSession[bytes]],
                self.forward_unix_connection(dest_path),
            )

        self.logger.info(
            "Creating remote UNIX forwarder from %s to %s", listen_path, dest_path
        )
    ...
    async def forward_remote_port_to_path(
        ...
        def session_factory(
            ...
            return cast(
                Awaitable[SSHUNIXSession], self.forward_unix_connection(dest_path)
            )
    ...
    async def forward_remote_path_to_port(
        ...
        def session_factory() -> Awaitable[SSHTCPSession[bytes]]:
            ...
            return cast(
                Awaitable[SSHTCPSession[bytes]],
                self.forward_connection(dest_host, dest_port),
            )

        self.logger.info(
            "Creating remote UNIX forwarder from %s to %s",
            listen_path,
            (dest_host, dest_port),
        )
    ...
    @async_context_manager
    async def forward_socks(self, listen_host: str, listen_port: int) -> SSHListener:
        ...
        self.logger.info(
            "Creating local SOCKS forwarder on %s", (listen_host, listen_port)
        )

        try:
            listener = await create_socks_listener(
                self, self._loop, tunnel_socks, listen_host, listen_port
            )
        except OSError as exc:
    ...
    @async_context_manager
    async def forward_tun(
        self, local_unit: Optional[int] = None, remote_unit: Optional[int] = None
    ) -> SSHForwarder:
        ...
        def session_factory() -> SSHTunTapSession:
            ...
            return cast(
                SSHTunTapSession,
                self.forward_tuntap(SSH_TUN_MODE_POINTTOPOINT, local_unit),
            )
    ...
    async def forward_tap(
        ...
        def session_factory() -> SSHTunTapSession:
            ...
            return cast(
                SSHTunTapSession, self.forward_tuntap(SSH_TUN_MODE_ETHERNET, local_unit)
            )
    ...
    async def start_sftp_client(
        ...
        writer, reader, _ = await self.open_session(
            subsystem="sftp", env=env, send_env=send_env, encoding=None
        )

        return await start_sftp_client(
            self, self._loop, reader, writer, path_encoding, path_errors, sftp_version
        )
...
class SSHServerConnection(SSHConnection):
    ...
    def __init__(
        ...
        super().__init__(loop, options, acceptor, error_handler, wait, server=True)
    ...
    async def reload_config(self) -> None:
        ...
        options = await SSHServerConnectionOptions.construct(
            ...
            reload=True,
            accept_addr=self._local_addr,
            accept_port=self._local_port,
            username=self._username,
            client_host=self._peer_host,
    ...
    def choose_server_host_key(self, peer_host_key_algs: Sequence[bytes]) -> bool:
    ...
    async def validate_gss_principal(
        self, username: str, user_principal: str, host_principal: str
    ) -> bool:
        ...
        result = self._owner.validate_gss_principal(
            username, user_principal, host_principal
        )
    ...
    def host_based_auth_supported(self) -> bool:
        ...
        return self._host_based_auth and (
            bool(self._known_client_hosts) or self._owner.host_based_auth_supported()
        )

    async def validate_host_based_auth(
        self,
        username: str,
        key_data: bytes,
        client_host: str,
        ...
        if self._trust_client_host:
            ...
            if resolved_host != client_host:
                self.logger.info(
                    "Client host mismatch: received %s, " "resolved %s",
                    client_host,
                    resolved_host,
                )
        ...
        if self._known_client_hosts:
            self._match_known_hosts(
                self._known_client_hosts, resolved_host, self._peer_addr, None
            )

        try:
            key = self._validate_host_key(
                resolved_host, self._peer_addr, self._peer_port, key_data
            )
        except ValueError as exc:
        ...
        result = self._owner.validate_host_based_user(
            username, client_host, client_username
        )
    ...
    async def _validate_openssh_certificate(
        ...
        allowed_addresses = cast(
            Sequence[IPNetwork], cert.options.get("source-address")
        )
        ...
        cert.key.set_touch_required(
            not (
                self.get_key_option("no-touch-required", False)
                and self.get_certificate_option("no-touch-required", False)
            )
        )
    ...
    async def _validate_x509_certificate_chain(
        ...
        options, trusted_cert = self._authorized_client_keys.validate_x509(
    ...
    async def _validate_client_certificate(
        self, username: str, key_data: bytes
    ) -> Optional[SSHKey]:
    ...
    async def _validate_client_public_key(
        self, username: str, key_data: bytes
    ) -> Optional[SSHKey]:
        ...
        key.set_touch_required(not self.get_key_option("no-touch-required", False))
    ...
    def public_key_auth_supported(self) -> bool:
        ...
        return self._public_key_auth and (
            bool(self._authorized_client_keys)
            or self._owner.public_key_auth_supported()
        )

    async def validate_public_key(
        self, username: str, key_data: bytes, msg: bytes, signature: bytes
    ) -> bool:
    ...
    async def change_password(
        ...
        result = self._owner.change_password(username, old_password, new_password)
    ...
    def kbdint_auth_supported(self) -> bool:
        ...
        if result is True:
            return True
        elif result is NotImplemented and self._owner.password_auth_supported():
            self._kbdint_password_auth = True
            return True
        else:
            return False
    ...
    async def get_kbdint_challenge(
        ...
        if self._kbdint_password_auth:
            challenge: KbdIntChallenge = ("", "", DEFAULT_LANG, (("Password:", False),))
        else:
            result = self._owner.get_kbdint_challenge(username, lang, submethods)
    ...
    async def validate_kbdint_response(
        ...
        if self._kbdint_password_auth:
            ...
            try:
                pw_result = self._owner.validate_password(username, responses[0])
    ...
    def _process_session_open(
        ...
        if self._process_factory or self._session_factory or self._sftp_factory:
            ...
            if self._process_factory:
                ...
                session = SSHServerStreamSession(
    ...
    def _process_direct_tcpip_open(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid direct TCP/IP channel " "open request"
            ) from None

        if not self.check_key_permission(
            "port-forwarding"
        ) or not self.check_certificate_permission("port-forwarding"):
            raise ChannelOpenError(
                OPEN_ADMINISTRATIVELY_PROHIBITED, "Port forwarding not permitted"
            )

        permitted_opens = cast(Set[Tuple[str, int]], self.get_key_option("permitopen"))

        if (
            permitted_opens
            and (dest_host, dest_port) not in permitted_opens
            and (dest_host, None) not in permitted_opens
        ):
            raise ChannelOpenError(
                OPEN_ADMINISTRATIVELY_PROHIBITED,
                "Port forwarding not permitted to " f"{dest_host} port {dest_port}",
            )

        result = self._owner.connection_requested(
        ...
        if result is True:
            result = cast(
                SSHTCPSession[bytes], self.forward_connection(dest_host, dest_port)
            )
        elif isinstance(result, SSHClientConnection):
            result = cast(
                Awaitable[SSHTCPSession[bytes]],
                self.forward_tunneled_connection(result, dest_host, dest_port),
            )
        ...
        self.logger.info(
            "Accepted direct TCP connection request to %s", (dest_host, dest_port)
        )
        self.logger.info("  Client address: %s", (orig_host, orig_port))
    ...
    def _process_tcpip_forward_global_request(self, packet: SSHPacket) -> None:
        ...
        if not self.check_key_permission(
            ...
            self.logger.info(
                "Request for TCP listener on %s denied: port "
    ...
    async def _finish_port_forward(self, listen_host: str, listen_port: int) -> None:
        ...
        try:
            ...
            if listener is True:
                ...
                listener = await self.forward_local_port(
                    listen_host, listen_port, listen_host, listen_port, listener
                )
        except OSError:
        ...
        if not listener:
            self.logger.info(
                "Request for TCP listener on %s denied by " "application",
                (listen_host, listen_port),
            )
        ...
        self.logger.info("Created TCP listener on %s", (listen_host, listen_port))

        self._local_listeners[listen_host, listen_port] = listener
        self._report_global_response(result)

    def _process_cancel_tcpip_forward_global_request(self, packet: SSHPacket) -> None:
        ...
        try:
            ...
            raise ProtocolError("Invalid TCP/IP cancel " "forward request") from None
        ...
        self.logger.info("Closed TCP listener on %s", (listen_host, listen_port))
    ...
    def _process_direct_streamlocal_at_openssh_dot_com_open(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid direct UNIX domain channel " "open request"
            ) from None
        ...
        if result is True:
            result = cast(
                SSHUNIXSession[bytes], self.forward_unix_connection(dest_path)
            )
        elif isinstance(result, SSHClientConnection):
            result = cast(
                Awaitable[SSHUNIXSession[bytes]],
                self.forward_tunneled_unix_connection(result, dest_path),
            )
    ...
    def _process_streamlocal_forward_at_openssh_dot_com_global_request(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid UNIX domain socket " "forward request"
            ) from None
        ...
        if not self.check_key_permission(
            ...
            self.logger.info(
                "Request for UNIX listener on %s denied: port "
                "forwarding not permitted",
                listen_path,
            )
    ...
    async def _finish_path_forward(self, listen_path: str) -> None:
        ...
        try:
            ...
            if listener is True:
                listener = await self.forward_local_path(listen_path, listen_path)
        except OSError:
        ...
        if not listener:
            self.logger.info(
                "Request for UNIX listener on %s denied by " "application", listen_path
            )
    ...
    def _process_cancel_streamlocal_forward_at_openssh_dot_com_global_request(
        ...
        try:
            ...
            raise ProtocolError(
                "Invalid UNIX domain cancel " "forward request"
            ) from None
    ...
    def _process_tun_at_openssh_dot_com_open(
        ...
        if not result:
            raise ChannelOpenError(OPEN_CONNECT_FAILED, "TUN/TAP request refused")
        ...
        self.logger.info(
            "Accepted layer %d tunnel request to unit %s",
            3 if mode == SSH_TUN_MODE_POINTTOPOINT else 2,
    ...
    def _process_hostkeys_prove_00_at_openssh_dot_com_global_request(
        ...
        prefix = String("hostkeys-prove-00@openssh.com") + String(self._session_id)
    ...
    async def attach_x11_listener(
        ...
        if (
            not self._x11_forwarding
            or not self.check_key_permission("X11-forwarding")
            or not self.check_certificate_permission("X11-forwarding")
        ):
            self.logger.info(
                "X11 forwarding request denied: X11 " "forwarding not permitted"
            )
    ...
    async def create_agent_listener(self) -> bool:
        ...
        if (
            not self._agent_forwarding
            or not self.check_key_permission("agent-forwarding")
            or not self.check_certificate_permission("agent-forwarding")
        ):
            self.logger.info(
                "Agent forwarding request denied: Agent " "forwarding not permitted"
            )
        ...
        try:
            ...
            self._agent_listener = SSHAgentListener(tempdir, path, unix_listener)
            return True
        except OSError:
            return False
    ...
    def get_certificate_option(self, option: str, default: object = None) -> object:
    ...
    def check_certificate_permission(self, permission: str) -> bool:
        ...
        if self._cert_options is not None:
            return cast(bool, self._cert_options.get("permit-" + permission, False))
        else:
            return True
    ...
    async def create_connection(
        ...
        *,
        ...
        self.logger.info(
            "Opening forwarded TCP connection to %s", (remote_host, remote_port)
        )
        self.logger.info("  Client address: %s", (orig_host, orig_port))

        chan = self.create_tcp_channel(encoding, errors, window, max_pktsize)

        session = await chan.accept(
            session_factory, remote_host, remote_port, orig_host, orig_port
        )
    ...
    async def create_agent_connection(
        ...
        *,
        ...
        if not self._agent_listener:
            raise ChannelOpenError(
                OPEN_ADMINISTRATIVELY_PROHIBITED, "Agent forwarding not permitted"
            )
    ...
    async def open_agent_connection(self) -> Tuple[SSHReader[bytes], SSHWriter[bytes]]:
        """Open a forwarded ssh-agent connection back to the client"""

        chan, session = await self.create_agent_connection(SSHUNIXStreamSession)
    ...
    async def forward_tunneled_connection(
        self, conn: SSHClientConnection, dest_host: str, dest_port: int
    ) -> SSHForwarder:
        """Forward a tunneled TCP connection between SSH connections"""

        _, peer = await conn.create_connection(
            cast(SSHTCPSessionFactory[bytes], SSHForwarder), dest_host, dest_port
        )

        self.logger.info(
            "  Forwarding TCP connection to %s via SSH tunnel", (dest_host, dest_port)
        )

        return SSHForwarder(cast(SSHForwarder, peer))

    async def forward_tunneled_unix_connection(
        self, conn: SSHClientConnection, dest_path: str
    ) -> SSHForwarder:
        ...
        self.logger.info("  Forwarding UNIX connection to %s via SSH tunnel", dest_path)

        return SSHForwarder(cast(SSHForwarder, peer))

    async def forward_tunneled_tuntap(
        self, conn: SSHClientConnection, mode: int, unit: Optional[int]
    ) -> SSHForwarder:
        ...
        self.logger.info(
            "  Forwarding layer %d traffic to %s via SSH tunnel", layer, interface
        )
...
class SSHConnectionOptions(Options, Generic[_Options]):
    ...
    @classmethod
    async def construct(
        cls, options: Optional[_Options] = None, **kwargs: object
    ) -> _Options:
    ...
    def prepare(
        ...
        kex_algs: _AlgsArg,
        encryption_algs: _AlgsArg,
        mac_algs: _AlgsArg,
        compression_algs: _AlgsArg,
        signature_algs: _AlgsArg,
        host_based_auth: _AuthArg,
        public_key_auth: _AuthArg,
        kbdint_auth: _AuthArg,
        ...
        def _split_cname_patterns(
            patterns: Union[str, Tuple[str, str]]
        ) -> Tuple[str, str]:
        ...
        self.port = cast(int, port if port != () else config.get("Port", DEFAULT_PORT))
        ...
        self.family = cast(
            int,
            family if family != () else config.get("AddressFamily", socket.AF_UNSPEC),
        )

        bind_addr = config.get("BindAddress")

        self.local_addr = cast(
            HostPort,
            local_addr if local_addr != () else (bind_addr, 0) if bind_addr else None,
        )

        self.tcp_keepalive = cast(
            bool,
            tcp_keepalive if tcp_keepalive != () else config.get("TCPKeepAlive", True),
        )

        self.canonicalize_hostname = cast(
            Union[bool, str],
        ...
        self.canonicalize_fallback_local = cast(
            bool,
        ...
        self.canonicalize_max_dots = cast(
            int,
            canonicalize_max_dots
            if canonicalize_max_dots != ()
            else config.get("CanonicalizeMaxDots", 1),
        )

        permitted_cnames = cast(
            Sequence[str],
        ...
        self.host_based_auth = cast(
            bool,
            host_based_auth
            if host_based_auth != ()
            else config.get("HostbasedAuthentication", True),
        )

        self.public_key_auth = cast(
            bool,
            public_key_auth
            if public_key_auth != ()
            else config.get("PubkeyAuthentication", True),
        )

        self.kbdint_auth = cast(
            bool,
            kbdint_auth
            if kbdint_auth != ()
            else config.get(
        ...
        self.password_auth = cast(
            bool,
            password_auth
            if password_auth != ()
            else config.get("PasswordAuthentication", True),
        )
        ...
        if x509_trusted_certs is not None:
            ...
            for cert in certs:
                if not cert.is_x509:
                    raise ValueError(
                        "OpenSSH certificates not allowed " "in X.509 trusted certs"
                    )
        ...
        if x509_trusted_cert_paths:
            for path in x509_trusted_cert_paths:
                if not Path(path).is_dir():
                    raise ValueError(
                        "X.509 trusted certificate path not " f"a directory: {path}"
                    )
        ...
        config_rekey_bytes, config_rekey_seconds = cast(
            Tuple[DefTuple[int], DefTuple[int]], config.get("RekeyLimit", ((), ()))
        )
...
class SSHClientConnectionOptions(SSHConnectionOptions):
    ...
    def prepare(
        ...
        config: DefTuple[ConfigPaths] = None,
        reload: bool = False,
        canonical: bool = False,
        final: bool = False,
        client_factory: Optional[_ClientFactory] = None,
        client_version: _VersionArg = (),
        host: str = "",
        port: DefTuple[int] = (),
        tunnel: object = (),
        proxy_command: DefTuple[_ProxyCommand] = (),
        ...
        try:
            ...
            raise ValueError(
                "Unknown local username: set one of "
        ...
        if config == () and (not last_config or not last_config.loaded):
            ...
            config = [default_config] if os.access(default_config, os.R_OK) else []
        ...
        if connect_timeout == ():
            connect_timeout = cast(
                Optional[Union[float, str]], config.get("ConnectTimeout", None)
            )

        connect_timeout: Optional[Union[float, str]]

        if keepalive_interval == ():
            keepalive_interval = cast(
                Union[float, str],
                config.get("ServerAliveInterval", _DEFAULT_KEEPALIVE_INTERVAL),
            )

        keepalive_interval: Union[float, str]

        if keepalive_count_max == ():
            keepalive_count_max = cast(
                int, config.get("ServerAliveCountMax", _DEFAULT_KEEPALIVE_COUNT_MAX)
            )
        ...
        if known_hosts != ():
            ...
            user_known_hosts = cast(List[str], config.get("UserKnownHostsFile", ()))

            if user_known_hosts == []:
                self.known_hosts = None
            else:
                self.known_hosts = list(user_known_hosts) + cast(
                    List[str], config.get("GlobalKnownHostsFile", [])
                )

        self.host_key_alias = cast(
            Optional[str],
            host_key_alias if host_key_alias != () else config.get("HostKeyAlias"),
        )
        ...
        self.username = saslprep(
            cast(
                str, username if username != () else config.get("User", local_username)
            )
        )
        ...
        if client_host_keysign == ():
            client_host_keysign = cast(bool, config.get("EnableSSHKeySign", False))

        if client_host_keysign:
            client_host_keysign = find_keysign(client_host_keysign)

            if client_host_keys:
                self.client_host_pubkeys = load_public_keys(
                    cast(KeyListArg, client_host_keys)
                )
            else:
            ...
            self.client_host_keypairs = load_keypairs(
                cast(KeyPairListArg, client_host_keys),
                passphrase,
                client_host_certs,
                loop=loop,
            )
        ...
        self.client_username = saslprep(
            cast(str, client_username if client_username != () else local_username)
        )
        ...
        self.gss_kex = cast(
            bool, gss_kex if gss_kex != () else config.get("GSSAPIKeyExchange", True)
        )

        self.gss_auth = cast(
            bool,
            gss_auth if gss_auth != () else config.get("GSSAPIAuthentication", True),
        )

        self.gss_delegate_creds = cast(
            bool,
            gss_delegate_creds
            if gss_delegate_creds != ()
            else config.get("GSSAPIDelegateCredentials", False),
        )

        if preferred_auth == ():
            preferred_auth = cast(str, config.get("PreferredAuthentications", ()))
        ...
        if pkcs11_provider == ():
            pkcs11_provider = cast(Optional[str], config.get("PKCS11Provider"))
        ...
        if client_certs == ():
            client_certs = cast(Sequence[FilePath], config.get("CertificateFile", ()))
        ...
        if agent_identities:
            self.agent_identities = load_identities(agent_identities, identities_only)
        ...
        if client_keys:
            self.client_keys = load_keypairs(
                cast(KeyPairListArg, client_keys),
                passphrase,
                client_certs,
                identities_only,
                ignore_encrypted,
        ...
        if agent_forwarding == ():
            agent_forwarding = cast(Union[bool, str], config.get("ForwardAgent", False))
        ...
        self.command = cast(
            Optional[str], command if command != () else config.get("RemoteCommand")
        )
        ...
        self.send_env = cast(
            Optional[EnvSeq], send_env if send_env != () else config.get("SendEnv")
        )

        self.request_pty = cast(
            _RequestPTY,
            request_pty if request_pty != () else config.get("RequestTTY", True),
        )
        ...
        self.x11_forwarding = cast(
            Union[bool, str],
            x11_forwarding
            if x11_forwarding != ()
            else config.get("ForwardX11Trusted") and "ignore_failure",
        )
...
class SSHServerConnectionOptions(SSHConnectionOptions):
    ...
    def prepare(
        ...
        if login_timeout == ():
            login_timeout = cast(
                Union[float, str], config.get("LoginGraceTime", _DEFAULT_LOGIN_TIMEOUT)
            )
        ...
        if keepalive_interval == ():
            keepalive_interval = cast(
                Union[float, str],
                config.get("ClientAliveInterval", _DEFAULT_KEEPALIVE_INTERVAL),
            )
        ...
        if keepalive_count_max == ():
            keepalive_count_max = cast(
                int, config.get("ClientAliveCountMax", _DEFAULT_KEEPALIVE_COUNT_MAX)
            )
        ...
        if server_host_certs == ():
            server_host_certs = cast(Sequence[str], config.get("HostCertificate", ()))

        server_keys = load_keypairs(
            server_host_keys, passphrase, server_host_certs, loop=loop
        )
        ...
        if authorized_client_keys == () and reload:
            authorized_client_keys = cast(List[str], config.get("AuthorizedKeysFile"))

        if isinstance(authorized_client_keys, (str, list)):
            self.authorized_client_keys = read_authorized_keys(authorized_client_keys)
        else:
        ...
        self.allow_pty = cast(
            bool, allow_pty if allow_pty != () else config.get("PermitTTY", True)
        )
        ...
        self.rdns_lookup = cast(
            bool, rdns_lookup if rdns_lookup != () else config.get("UseDNS", False)
        )

        self.x11_forwarding = x11_forwarding
        self.x11_auth_path = x11_auth_path

        self.agent_forwarding = cast(
            bool,
            agent_forwarding
            if agent_forwarding != ()
            else config.get("AllowAgentForwarding", True),
        )
...
async def run_client(
    ...
    **kwargs: object,
    ...
    return await asyncio.wait_for(
        _connect(
            new_options, config, loop, 0, sock, conn_factory, "Starting SSH client on"
        ),
...
async def run_server(
    ...
    **kwargs: object,
    ...
    return await asyncio.wait_for(
        _connect(
            new_options, config, loop, 0, sock, conn_factory, "Starting SSH server on"
        ),
        timeout=new_options.connect_timeout,
    )
...
async def create_connection(
    client_factory: _ClientFactory, host="", port: DefTuple[int] = (), **kwargs: object
) -> Tuple[SSHClientConnection, SSHClient]:
    ...
    conn = await connect(
        host, port, client_factory=client_factory, **kwargs
    )  # type: ignore
...
@async_context_manager
async def create_server(
    server_factory: _ServerFactory, host="", port: DefTuple[int] = (), **kwargs: object
) -> SSHAcceptor:
    ...
    return await listen(
        host, port, server_factory=server_factory, **kwargs
    )  # type: ignore
