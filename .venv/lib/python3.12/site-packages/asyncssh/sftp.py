...
_SFTPPacketHandler = Optional[
    Callable[["SFTPServerHandler", SSHPacket], Awaitable[object]]
]
...
_file_types = {
    k: v.lower() for k, v in get_symbol_names(_const_dict, "FILEXFER_TYPE_", 14).items()
}
...
class _SFTPFSProtocol(Protocol):
    ...
    def compose_path(self, path: bytes, parent: Optional[bytes] = None) -> bytes:
        """Compose a path"""

    async def stat(self, path: bytes, *, follow_symlinks: bool = True) -> "SFTPAttrs":
        """Get attributes of a file, directory, or symlink"""

    async def setstat(
        self, path: bytes, attrs: "SFTPAttrs", *, follow_symlinks: bool = True
    ) -> None:
    ...
    async def open(
        self, path: bytes, mode: str, block_size: int = -1
    ) -> SFTPFileProtocol:
        """Open a file"""
...
def _parse_supported(data: bytes) -> Tuple[int, int, int, int, int, Sequence[bytes]]:
    ...
    return (attr_mask, attrib_mask, open_flags, access_mask, max_read_size, ext_names)


def _parse_supported2(
    data: bytes,
) -> Tuple[int, int, int, int, int, int, int, Sequence[bytes], Sequence[bytes]]:
...
def _utime_to_attrs(
    times: Optional[Tuple[float, float]] = None, ns: Optional[Tuple[int, int]] = None
) -> "SFTPAttrs":
    ...
    return SFTPAttrs(atime=atime, atime_ns=atime_ns, mtime=mtime, mtime_ns=mtime_ns)
...
def _setstat(
    path: Union[int, _SFTPPath], attrs: "SFTPAttrs", *, follow_symlinks: bool = True
) -> None:
    ...
    atime_ns = (
        _tuple_to_nsec(attrs.atime, attrs.atime_ns) if attrs.atime is not None else None
    )

    mtime_ns = (
        _tuple_to_nsec(attrs.mtime, attrs.mtime_ns) if attrs.mtime is not None else None
    )
    ...
    if attrs.permissions is not None:
        try:
            os.chmod(
                path, stat.S_IMODE(attrs.permissions), follow_symlinks=follow_symlinks
            )
        except NotImplementedError:  # pragma: no cover
            pass
...
if sys.platform == "win32" and _pywin32_available:  # pragma: no cover
    ...
    async def _request_ranges(
        file_obj: _SFTPFileObj, offset: int, length: int
    ) -> AsyncIterator[Tuple[int, int]]:
        ...
        while True:
            try:
                query_range = offset.to_bytes(8, "little") + length.to_bytes(
                    8, "little"
                )

                ranges = win32file.DeviceIoControl(
                    handle,
                    winioctlcon.FSCTL_QUERY_ALLOCATED_RANGES,
                    query_range,
                    bufsize,
                    None,
                )
...
class _SFTPParallelIO(Generic[_T]):
    ...
    def __init__(self, block_size: int, max_requests: int, offset: int, size: int):
    ...
    async def _start_task(self, offset: int, size: int) -> Tuple[int, int, int, _T]:
...
class _SFTPFileWriter(_SFTPParallelIO[int]):
    ...
    async def run_task(self, offset: int, size: int) -> Tuple[int, int]:
        ...
        pos = offset - self._start
        await self._handler.write(self._handle, offset, self._data[pos : pos + size])
        return size, size
...
class _SFTPFileCopier(_SFTPParallelIO[int]):
    ...
    def __init__(
        ...
        total_bytes: int,
        sparse: bool,
        srcfs: _SFTPFSProtocol,
        dstfs: _SFTPFSProtocol,
        srcpath: bytes,
        dstpath: bytes,
    ...
    async def run(self) -> None:
        ...
        async def _request_nonsparse_range(
        ...
        try:
            self._src = await self._srcfs.open(self._srcpath, "rb", block_size=0)
            self._dst = await self._dstfs.open(self._dstpath, "wb", block_size=0)
            ...
            if (
                ...
                and isinstance(self._srcfs, SFTPClient)
...
class SFTPError(Error):
    ...
    def encode(self, version: int) -> bytes:
        ...
        if self.code == FX_NOT_A_DIRECTORY and version < 6:
            ...
            (self.code > FX_V3_END and version <= 3)
            or (self.code > FX_V4_END and version <= 4)
            or (self.code > FX_V5_END and version <= 5)
        ):
            code = FX_FAILURE
        else:
            code = self.code
...
class SFTPUnknownPrincipal(SFTPError):
    ...
    def __init__(
        self, reason: str, lang: str = DEFAULT_LANG, unknown_names: Sequence[str] = ()
    ):
    ...
    def encode(self, version: int) -> bytes:
        ...
        return super().encode(version) + b"".join(
            String(name) for name in self.unknown_names
        )
    ...
    def decode(self, packet: SSHPacket) -> None:
        ...
        try:
            while packet:
                self.unknown_names.append(packet.get_string().decode("utf-8"))
        except UnicodeDecodeError:
            raise SFTPBadMessage("Invalid status message") from None
...
class SFTPAttrs(Record):
    ...
    def _format(self, k: str, v: object) -> Optional[str]:
        ...
        if k == "type":
            return (
                _file_types.get(cast(int, v), str(v))
                if v != FILEXFER_TYPE_UNKNOWN
                else None
            )
        elif k == "permissions":
    ...
    def encode(self, sftp_version: int) -> bytes:
        ...
        if sftp_version == 3:
            if self.uid is not None and self.gid is not None:
                ...
                raise ValueError("Setting owner and group requires SFTPv4 " "or later")
        else:
        ...
        if sftp_version == 3:
            ...
            subsecond = (
                self.atime_ns is not None
                or self.crtime_ns is not None
                or self.mtime_ns is not None
                or self.ctime_ns is not None
            )
        ...
        if (
            sftp_version >= 5
            and self.attrib_bits is not None
            and self.attrib_valid is not None
        ):
        ...
        if self.extended:
            ...
            attrs.extend(String(type) + String(data) for type, data in self.extended)
    ...
    def decode(cls, packet: SSHPacket, sftp_version: int) -> "SFTPAttrs":
        ...
        # SFTPv4 and later.
        if sftp_version == 3 and flags & (
            FILEXFER_ATTR_ACMODTIME | FILEXFER_ATTR_MODIFYTIME
        ):
            flags &= ~FILEXFER_ATTR_MODIFYTIME
            ...
            if flags & FILEXFER_ATTR_OWNERGROUP:
                ...
                try:
                    ...
                    raise SFTPOwnerInvalid(
                        "Invalid owner name: "
                        + owner.decode("utf-8", "backslashreplace")
                    ) from None
                ...
                try:
                    ...
                    raise SFTPGroupInvalid(
                        "Invalid group name: "
                        + group.decode("utf-8", "backslashreplace")
                    ) from None
...
class SFTPName(Record):
    ...
    def encode(self, sftp_version: int) -> bytes:
        ...
        return String(self.filename) + longname + self.attrs.encode(sftp_version)
...
class SFTPLimits(Record):
    ...
    def encode(self) -> bytes:
        ...
        return (
            UInt64(self.max_packet_len)
            + UInt64(self.max_read_len)
            + UInt64(self.max_write_len)
            + UInt64(self.max_open_handles)
        )
    ...
    def decode(cls, packet: SSHPacket) -> Self:
        ...
        return cls(max_packet_len, max_read_len, max_write_len, max_open_handles)
...
class SFTPRanges(Record):
    ...
    def encode(self) -> bytes:
        ...
        return (
            UInt32(len(self.ranges))
            + b"".join(
                (UInt64(offset) + UInt64(length) for offset, length in self.ranges)
            )
            + Boolean(self.at_end)
        )
    ...
    def decode(cls, packet: SSHPacket) -> Self:
        ...
        count = packet.get_uint32()
        ranges = [(packet.get_uint64(), packet.get_uint64()) for _ in range(count)]
...
class SFTPHandler(SSHPacketLogger):
    ...
    def _log_extensions(self, extensions: Sequence[Tuple[bytes, bytes]]):
        ...
        for name, data in extensions:
            if name == b"acl-supported":
                ...
                self.logger.debug1("    access_mask: 0x%08x", access_mask)
                self.logger.debug1("    max_read_size: %d", max_read_size)
                self.logger.debug1("    open_block_vector: 0x%04x", open_block_vector)
                ...
                self.logger.debug1("  %s%s%s", name, ": " if data else "", data)
    ...
    def send_packet(self, pkttype: int, pktid: Optional[int], *args: bytes) -> None:
        """Send an SFTP packet"""
...
class SFTPClientHandler(SFTPHandler):
    ...
    def _send_request(
        self, pkttype: Union[int, bytes], args: Sequence[bytes], waiter: _RequestWaiter
    ) -> None:
    ...
    async def _make_request(self, pkttype: Union[int, bytes], *args: bytes) -> object:
    ...
    def _process_data(self, packet: SSHPacket) -> Tuple[bytes, bool]:
        ...
        data = packet.get_string()
        at_end = packet.get_boolean() if packet and self._version >= 6 else False
    ...
    def _process_name(self, packet: SSHPacket) -> _SFTPNames:
        ...
        self.logger.debug1(
            "Received %s%s", plural(count, "name"), " (at end)" if at_end else ""
        )
    ...
    async def start(self) -> None:
        ...
        self.logger.debug1(
            "Received version=%d%s", version, ", extensions:" if rcvd_extensions else ""
        )
        ...
        if version == 3:
            ...
            server_version = cast(
                str, self._reader.get_extra_info("server_version", "")
            )

            if any(name in server_version for name in self._nonstandard_symlink_impls):
                self.logger.debug1(
                    "Adjusting for non-standard symlink " "implementation"
                )
    ...
    async def request_limits(self) -> None:
        ...
        if self._supports_limits:
            packet = cast(SSHPacket, await self._make_request(b"limits@openssh.com"))
    ...
    async def open(self, filename: bytes, pflags: int, attrs: SFTPAttrs) -> bytes:
        ...
        if self._version >= 5:
            ...
            self.logger.debug1(
                "Sending open for %s, desired_access=0x%08x, " "flags=0x%08x%s",
                filename,
                desired_access,
                flags,
                hide_empty(attrs),
            ...
            self.logger.debug1(
                "Sending open for %s, mode 0x%02x%s",
                filename,
                pflags,
                hide_empty(attrs),
            )
    ...
    async def open56(
        self, filename: bytes, desired_access: int, flags: int, attrs: SFTPAttrs
    ) -> bytes:
    ...
    async def read(self, handle: bytes, offset: int, length: int) -> Tuple[bytes, bool]:
        """Make an SFTP read request"""

        self.logger.debug1(
            "Sending read for %s at offset %d in handle %s",
            plural(length, "byte"),
            offset,
            handle.hex(),
        )
    ...
    async def write(self, handle: bytes, offset: int, data: bytes) -> int:
        ...
        self.logger.debug1(
            "Sending write for %s at offset %d in handle %s",
            plural(len(data), "byte"),
            offset,
            handle.hex(),
        )
    ...
    async def stat(
        self, path: bytes, flags: int, *, follow_symlinks: bool = True
    ) -> SFTPAttrs:
        ...
        if follow_symlinks:
            ...
            return cast(
                SFTPAttrs, await self._make_request(FXP_STAT, String(path), flag_bytes)
            )
        else:
            self.logger.debug1("Sending lstat for %s%s", path, flag_text)

            return cast(
                SFTPAttrs, await self._make_request(FXP_LSTAT, String(path), flag_bytes)
            )
    ...
    async def fstat(self, handle: bytes, flags: int) -> SFTPAttrs:
        ...
        self.logger.debug1("Sending fstat for handle %s%s", handle.hex(), flag_text)

        return cast(
            SFTPAttrs, await self._make_request(FXP_FSTAT, String(handle), flag_bytes)
        )
    ...
    async def setstat(
        ...
        if follow_symlinks:
            ...
            await self._make_request(
                FXP_SETSTAT, String(path), attrs.encode(self._version)
            )
        elif self._supports_lsetstat:
            self.logger.debug1("Sending lsetstat for %s%s", path, hide_empty(attrs))

            await self._make_request(
                b"lsetstat@openssh.com", String(path), attrs.encode(self._version)
            )
        else:
    ...
    async def fsetstat(self, handle: bytes, attrs: SFTPAttrs) -> None:
        ...
        self.logger.debug1(
            "Sending fsetstat for handle %s%s", handle.hex(), hide_empty(attrs)
        )

        await self._make_request(
            FXP_FSETSTAT, String(handle), attrs.encode(self._version)
        )
    ...
    async def statvfs(self, path: bytes) -> SFTPVFSAttrs:
        ...
        if self._supports_statvfs:
            ...
            packet = cast(
                SSHPacket,
                await self._make_request(b"statvfs@openssh.com", String(path)),
            )
    ...
    async def fstatvfs(self, handle: bytes) -> SFTPVFSAttrs:
        ...
        if self._supports_fstatvfs:
            ...
            packet = cast(
                SSHPacket,
                await self._make_request(b"fstatvfs@openssh.com", String(handle)),
            )
    ...
    async def rename(self, oldpath: bytes, newpath: bytes, flags: int) -> None:
        ...
        if self._version >= 5:
            self.logger.debug1(
                "Sending rename request from %s to %s%s",
                oldpath,
                newpath,
                f", flags=0x{flags:x}" if flags else "",
            )

            await self._make_request(
                FXP_RENAME, String(oldpath), String(newpath), UInt32(flags)
            )
        elif flags and self._supports_posix_rename:
            self.logger.debug1(
                "Sending OpenSSH POSIX rename request " "from %s to %s",
                oldpath,
                newpath,
            )

            await self._make_request(
                b"posix-rename@openssh.com", String(oldpath), String(newpath)
            )
        elif not flags:
            self.logger.debug1("Sending rename request from %s to %s", oldpath, newpath)
    ...
    async def posix_rename(self, oldpath: bytes, newpath: bytes) -> None:
        ...
        if self._supports_posix_rename:
            ...
            self.logger.debug1(
                "Sending rename request from %s to %s " "with overwrite",
                oldpath,
                newpath,
            )

            await self._make_request(
                FXP_RENAME, String(oldpath), String(newpath), UInt32(FXR_OVERWRITE)
            )
        else:
    ...
    async def opendir(self, path: bytes) -> bytes:
        ...
        return cast(bytes, await self._make_request(FXP_OPENDIR, String(path)))
    ...
    async def readdir(self, handle: bytes) -> _SFTPNames:
        ...
        return cast(_SFTPNames, await self._make_request(FXP_READDIR, String(handle)))
    ...
    async def mkdir(self, path: bytes, attrs: SFTPAttrs) -> None:
        ...
        await self._make_request(FXP_MKDIR, String(path), attrs.encode(self._version))
    ...
    async def realpath(
        self, path: bytes, *compose_paths: bytes, check: int = FXRP_NO_CHECK
    ) -> _SFTPNames:
        ...
        self.logger.debug1(
            "Sending realpath of %s%s%s",
            path,
            b", compose_path: " + b", ".join(compose_paths) if compose_paths else b"",
            checkmsg,
        )
        ...
        if self._version >= 6:
            return cast(
                _SFTPNames,
                await self._make_request(
                    FXP_REALPATH, String(path), Byte(check), *map(String, compose_paths)
                ),
            )
        else:
            return cast(
                _SFTPNames, await self._make_request(FXP_REALPATH, String(path))
            )
    ...
    async def readlink(self, path: bytes) -> _SFTPNames:
        ...
        return cast(_SFTPNames, await self._make_request(FXP_READLINK, String(path)))

    async def symlink(self, oldpath: bytes, newpath: bytes) -> None:
        """Make an SFTP symlink request"""

        self.logger.debug1("Sending symlink request from %s to %s", oldpath, newpath)

        if self._version >= 6:
            await self._make_request(
                FXP_LINK, String(newpath), String(oldpath), Boolean(True)
            )
        else:
    ...
    async def link(self, oldpath: bytes, newpath: bytes) -> None:
        ...
        if self._version >= 6 or self._supports_hardlink:
            self.logger.debug1(
                "Sending hardlink request from %s to %s", oldpath, newpath
            )

            if self._version >= 6:
                await self._make_request(
                    FXP_LINK, String(newpath), String(oldpath), Boolean(False)
                )
            else:
                await self._make_request(
    ...
    async def lock(self, handle: bytes, offset: int, length: int, flags: int) -> None:
        """Make an SFTP byte range lock request"""

        if self._version >= 6:
            self.logger.debug1(
                "Sending byte range lock request for "
            ...
            await self._make_request(
                FXP_BLOCK, String(handle), UInt64(offset), UInt64(length), UInt32(flags)
            )
        else:
    ...
    async def unlock(self, handle: bytes, offset: int, length: int) -> None:
        ...
        if self._version >= 6:
            self.logger.debug1(
                "Sending byte range unlock request for "
                "handle %s, offset %d, length %d",
                handle.hex(),
                offset,
                length,
            ...
            await self._make_request(
                FXP_UNBLOCK, String(handle), UInt64(offset), UInt64(length)
            )
        else:
            raise SFTPOpUnsupported("Byte range locks not supported")
    ...
    async def copy_data(
        self,
        read_from_handle: bytes,
        read_from_offset: int,
        read_from_length: int,
        write_to_handle: bytes,
        write_to_offset: int,
    ) -> None:
        ...
        if self._supports_copy_data:
            self.logger.debug1(
                "Sending copy-data from handle %s, "
                "offset %d, length %d to handle %s, "
                "offset %d",
                read_from_handle.hex(),
                read_from_offset,
                read_from_length,
                write_to_handle.hex(),
                write_to_offset,
    ...
    async def request_ranges(
        self, handle: bytes, offset: int, length: int
    ) -> SFTPRanges:
        """Request file ranges containing data in a remote file"""

        if self._supports_ranges:
            self.logger.debug1(
                "Sending ranges request for handle %s, " "offset %d, length %d",
                handle.hex(),
                offset,
                length,
            )
...
class SFTPClientFile:
    ...
    def __init__(
        ...
        self.read_len = handler.limits.max_read_len if block_size == -1 else block_size
        self.write_len = (
        ...
        if max_requests <= 0:
            if self.read_len:
                max_requests = max(16, min(MAX_SFTP_READ_LEN // self.read_len, 128))
            else:
                max_requests = 1
    ...
    async def read(self, size: int = -1, offset: Optional[int] = None) -> AnyStr:
        ...
        if offset is not None:
            ...
            try:
                if self.read_len and size > min(
                    self.read_len, self._handler.limits.max_read_len
                ):
    ...
    async def read_parallel(
        self, size: int = -1, offset: Optional[int] = None
        ...
        return _SFTPFileReader(
            self.read_len, self._max_requests, self._handler, self._handle, offset, size
        ).iter()
    ...
    @overload
    async def chown(self, owner: str, group: str) -> None:
        ...  # pragma: no cover
    ...
    async def chown(
        ...
        await self.setstat(SFTPAttrs(uid=uid, gid=gid, owner=owner, group=group))
...
class SFTPClient:
    ...
    def __init__(
        self, handler: SFTPClientHandler, path_encoding: Optional[str], path_errors: str
    ):
        self._handler = handler
    ...
    def encode(self, path: _SFTPPath) -> bytes:
        ...
        if isinstance(path, str):
            if self._path_encoding:
                ...
                raise SFTPBadMessage("Path must be bytes when " "encoding is not set")
    ...
    def compose_path(self, path: _SFTPPath, parent: Optional[bytes] = None) -> bytes:
        """Compose a path
    ...
    async def _type(
        self, path: _SFTPPath, statfunc: Optional[_SFTPStatFunc] = None
    ) -> int:
    ...
    async def _copy(
        ...
        try:
            ...
            if filetype == FILEXFER_TYPE_DIRECTORY:
                if not recurse:
                    exc = SFTPFileIsADirectory if self.version >= 6 else SFTPFailure

                    raise exc(
                        srcpath.decode("utf-8", "backslashreplace") + " is a directory"
                    )

                self.logger.info(
                    "  Starting copy of directory %s to %s", srcpath, dstpath
                )
                ...
                self.logger.info(
                    "  Finished copy of directory %s to %s", srcpath, dstpath
                )
            ...
            if preserve:
                attrs = await srcfs.stat(srcpath, follow_symlinks=follow_symlinks)
                ...
                try:
                    await dstfs.setstat(
                        ...
                        or filetype != FILEXFER_TYPE_SYMLINK,
                    )
    ...
    async def _begin_copy(
        ...
        if block_size <= 0:
            block_size = min(srcfs.limits.max_read_len, dstfs.limits.max_write_len)
        ...
        self.logger.info("Starting SFTP %s of %s to %s", copy_type, srcpaths, dstpath)
        ...
        if expand_glob:
            ...
            for srcpath in srcpaths:
                srcnames.extend(
                    await glob.match(srcfs.encode(srcpath), error_handler, self.version)
                )
        else:
            for srcpath in srcpaths:
                srcpath = srcfs.encode(srcpath)
                srcattrs = await srcfs.stat(srcpath, follow_symlinks=follow_symlinks)
        ...
        if len(srcnames) > 1 and not dst_isdir:
            ...
            raise exc(
                dstpath.decode("utf-8", "backslashreplace") + " must be a directory"
            )
    ...
    async def remote_copy(
        self,
        src: _SFTPClientFileOrPath,
        dst: _SFTPClientFileOrPath,
        src_offset: int = 0,
        src_length: int = 0,
        dst_offset: int = 0,
    ) -> None:
        ...
        await self._handler.copy_data(
            src.handle, src_offset, src_length, dst.handle, dst_offset
        )

    async def glob(
        self, patterns: _SFTPPaths, error_handler: SFTPErrorHandler = None
    ) -> Sequence[BytesOrStr]:
        ...
        return [
            name.filename for name in await self.glob_sftpname(patterns, error_handler)
        ]
    ...
    async def glob_sftpname(
        ...
        for pattern in patterns:
            new_matches = await glob.match(
                self.encode(pattern), error_handler, self.version
            )
    ...
    async def makedirs(
        self, path: _SFTPPath, attrs: SFTPAttrs = SFTPAttrs(), exist_ok: bool = False
    ) -> None:
        ...
        if exists and not exist_ok:
            ...
            raise exc(curpath.decode("utf-8", "backslashreplace") + " already exists")

    async def rmtree(
        self,
        path: _SFTPPath,
        ignore_errors: bool = False,
        onerror: _SFTPOnErrorHandler = None,
    ) -> None:
        ...
        try:
            if await self.islink(path):
                raise SFTPNoSuchFile(
                    path.decode("utf-8", "backslashreplace") + " must not be a symlink"
                )
        except SFTPError:
    ...
    async def open56(
        ...
        return SFTPClientFile(
            self._handler,
            handle,
            bool(desired_access & ACE4_APPEND_DATA or flags & FXF_APPEND_DATA),
            encoding,
            errors,
            block_size,
            max_requests,
    ...
    async def stat(
        ...
        *,
        ...
        path = self.compose_path(path)
        return await self._handler.stat(path, flags, follow_symlinks=follow_symlinks)

    async def lstat(self, path: _SFTPPath, flags=FILEXFER_ATTR_DEFINED_V4) -> SFTPAttrs:
        """Get attributes of a remote file, directory, or symlink
    ...
    async def setstat(
        self, path: _SFTPPath, attrs: SFTPAttrs, *, follow_symlinks: bool = True
    ) -> None:
        ...
        await self._handler.setstat(path, attrs, follow_symlinks=follow_symlinks)
    ...
    @overload
    async def chown(
        self, path: _SFTPPath, uid: int, gid: int, *, follow_symlinks: bool = True
    ) -> None:
        ...  # pragma: no cover

    @overload
    async def chown(
        self, path: _SFTPPath, owner: str, group: str, *, follow_symlinks: bool = True
    ) -> None:
        ...  # pragma: no cover
    ...
    async def chown(
        ...
        *,
        ...
        await self.setstat(
            path,
            SFTPAttrs(uid=uid, gid=gid, owner=owner, group=group),
            follow_symlinks=follow_symlinks,
        )

    async def chmod(
        self, path: _SFTPPath, mode: int, *, follow_symlinks: bool = True
    ) -> None:
        ...
        await self.setstat(
            path, SFTPAttrs(permissions=mode), follow_symlinks=follow_symlinks
        )
    ...
    async def utime(
        ...
        *,
        ...
        await self.setstat(
            path, _utime_to_attrs(times, ns), follow_symlinks=follow_symlinks
        )
    ...
    async def lexists(self, path: _SFTPPath) -> bool:
        ...
        return await self._type(path, statfunc=self.lstat) != FILEXFER_TYPE_UNKNOWN
    ...
    async def getatime(self, path: _SFTPPath) -> Optional[float]:
        ...
        return (
            _tuple_to_float_sec(attrs.atime, attrs.atime_ns)
    ...
    async def getatime_ns(self, path: _SFTPPath) -> Optional[int]:
        ...
        return (
            _tuple_to_nsec(attrs.atime, attrs.atime_ns)
            if attrs.atime is not None
            else None
        )
    ...
    async def getcrtime(self, path: _SFTPPath) -> Optional[float]:
        ...
        return (
            _tuple_to_float_sec(attrs.crtime, attrs.crtime_ns)
    ...
    async def getcrtime_ns(self, path: _SFTPPath) -> Optional[int]:
        ...
        return (
            _tuple_to_nsec(attrs.crtime, attrs.crtime_ns)
            if attrs.crtime is not None
            else None
        )
    ...
    async def getmtime(self, path: _SFTPPath) -> Optional[float]:
        ...
        return (
            _tuple_to_float_sec(attrs.mtime, attrs.mtime_ns)
    ...
    async def getmtime_ns(self, path: _SFTPPath) -> Optional[int]:
        ...
        return (
            _tuple_to_nsec(attrs.mtime, attrs.mtime_ns)
            if attrs.mtime is not None
            else None
        )
    ...
    async def rename(
        self, oldpath: _SFTPPath, newpath: _SFTPPath, flags: int = 0
    ) -> None:
    ...
    async def posix_rename(self, oldpath: _SFTPPath, newpath: _SFTPPath) -> None:
            ...
            while not at_end:
                ...
                for entry in names:
                    if isinstance(path, (str, PurePath)):
                        entry.filename = self.decode(cast(bytes, entry.filename))

                        if entry.longname is not None:
                            entry.longname = self.decode(cast(bytes, entry.longname))
    ...
    async def mkdir(self, path: _SFTPPath, attrs: SFTPAttrs = SFTPAttrs()) -> None:
        """Create a remote directory with the specified attributes
    ...
    @overload
    async def realpath(
        self, path: bytes, *compose_paths: bytes  # pragma: no cover
    ) -> bytes:
        ...

    @overload
    async def realpath(
        self, path: FilePath, *compose_paths: FilePath  # pragma: no cover
    ) -> str:
        ...

    @overload
    async def realpath(
        self, path: bytes, *compose_paths: bytes, check: int  # pragma: no cover
    ) -> SFTPName:
        ...

    @overload
    async def realpath(
        self, path: FilePath, *compose_paths: FilePath, check: int  # pragma: no cover
    ) -> SFTPName:
        ...

    async def realpath(
        self, path: _SFTPPath, *compose_paths: _SFTPPath, check: int = FXRP_NO_CHECK
        ...
        if check != FXRP_NO_CHECK:
            if self.version < 6:
                try:
                    names[0].attrs = await self._handler.stat(
                        self.encode(names[0].filename), _valid_attr_flags[self.version]
                    )
                except SFTPError:
            ...
            return self.decode(
                cast(bytes, names[0].filename), isinstance(path, (str, PurePath))
            )
    ...
    @overload
    async def readlink(self, path: FilePath) -> str:
        ...  # pragma: no cover
...
class SFTPServerHandler(SFTPHandler):
    ...
    _vendor_id = (
        String(__author__) + String("AsyncSSH") + String(__version__) + UInt64(0)
    )
    ...
    if hasattr(os, "statvfs"):  # pragma: no branch
        _extensions += [(b"statvfs@openssh.com", b"2"), (b"fstatvfs@openssh.com", b"2")]
    ...
    def _get_next_handle(self) -> bytes:
        ...
        while True:
            ...
            if handle not in self._file_handles and handle not in self._dir_handles:
                return handle
    ...
    async def _process_packet(
        ...
        try:
            ...
            if return_type == FXP_STATUS:
                ...
                response = (
                    UInt32(len(names))
                    + b"".join(name.encode(self._version) for name in names)
                    + end
                )
                ...
                if isinstance(result, os.stat_result):
                    ...
                    attrs = SFTPVFSAttrs.from_local(cast(os.statvfs_result, result))
                else:
            ...
            response = UInt32(FX_BAD_MESSAGE) + String(str(exc)) + String(DEFAULT_LANG)
            ...
            if exc.code == FX_EOF:
                ...
                self.logger.debug1(
                    "Sending %s: %s", exc.__class__.__name__, str(exc.reason)
                )
            ...
            if exc.errno == errno.ENOENT:
                ...
                self.logger.debug1("Sending no space on " "filesystem: %s", reason)
            ...
            self.logger.debug1("Sending failure: %s", reason, exc_info=sys.exc_info)

            response = UInt32(FX_FAILURE) + String(reason) + String(DEFAULT_LANG)
    ...
    async def _process_open(self, packet: SSHPacket) -> bytes:
        ...
        self.logger.debug1(
            "Received open request for %s, %s%s", path, flagmsg, hide_empty(attrs)
        )
    ...
    async def _process_read(self, packet: SSHPacket) -> Tuple[bytes, bool]:
        ...
        self.logger.debug1(
            "Received read for %s at offset %d in handle %s",
            plural(length, "byte"),
            offset,
            handle.hex(),
        )
        ...
        if file_obj:
            ...
            if self._version >= 6:
                attrs = await self._server.convert_attrs(self._server.fstat(file_obj))
    ...
    async def _process_write(self, packet: SSHPacket) -> int:
        ...
        self.logger.debug1(
            "Received write for %s at offset %d in handle %s",
            plural(len(data), "byte"),
            offset,
            handle.hex(),
        )
    ...
    async def _process_lstat(self, packet: SSHPacket) -> _SFTPOSAttrs:
        ...
        self.logger.debug1(
            "Received lstat for %s%s", path, f", flags=0x{flags:08x}" if flags else ""
        )
    ...
    async def _process_fsetstat(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received fsetstat for handle %s%s", handle.hex(), hide_empty(attrs)
        )
    ...
    async def _process_realpath(self, packet: SSHPacket) -> _SFTPNames:
        ...
        if check != FXRP_NO_CHECK:
            try:
                attrs = await self._server.convert_attrs(self._server.stat(result))
    ...
    async def _process_stat(self, packet: SSHPacket) -> _SFTPOSAttrs:
        ...
        self.logger.debug1(
            "Received stat for %s%s", path, f", flags=0x{flags:08x}" if flags else ""
        )
    ...
    async def _process_rename(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received rename request from %s to %s%s", oldpath, newpath, flag_text
        )
    ...
    async def _process_symlink(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1("Received symlink request from %s to %s", oldpath, newpath)
    ...
    async def _process_link(self, packet: SSHPacket) -> None:
        ...
        if symlink:
            self.logger.debug1(
                "Received symlink request from %s to %s", oldpath, newpath
            )

            result = self._server.symlink(oldpath, newpath)
        else:
            self.logger.debug1(
                "Received hardlink request from %s to %s", oldpath, newpath
            )
    ...
    async def _process_lock(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received byte range lock request for "
            "handle %s, offset %d, length %d, "
            "flags 0x%04x",
            handle.hex(),
            offset,
    ...
    async def _process_unlock(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received byte range lock request for " "handle %s, offset %d, length %d",
            handle.hex(),
            offset,
            length,
        )
    ...
    async def _process_posix_rename(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received POSIX rename request from %s to %s", oldpath, newpath
        )
    ...
    async def _process_openssh_link(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1("Received hardlink request from %s to %s", oldpath, newpath)
    ...
    async def _process_lsetstat(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1("Received lsetstat for %s%s", path, hide_empty(attrs))
    ...
    async def _process_limits(self, packet: SSHPacket) -> SFTPLimits:
        ...
        return SFTPLimits(
            MAX_SFTP_PACKET_LEN, MAX_SFTP_READ_LEN, MAX_SFTP_WRITE_LEN, nfiles
        )
    ...
    async def _process_copy_data(self, packet: SSHPacket) -> None:
        ...
        self.logger.debug1(
            "Received copy-data from handle %s, "
            "offset %d, length %d to handle %s, "
            "offset %d",
            read_from_handle.hex(),
            read_from_offset,
    ...
    async def _process_ranges(self, packet: SSHPacket) -> SFTPRanges:
        ...
        self.logger.debug1(
            "Received ranges request for handle %s, " "offset %d, length %d",
            handle.hex(),
            offset,
            length,
        )
    ...
    async def run(self) -> None:
        ...
        attrib_ext_names = b"".join(String(name) for name in self._attrib_extensions)

        if self._version == 5:
            supported = (
                UInt32(self._supported_attr_mask)
        ...
        self.logger.debug1(
            "Sending version=%d%s", self._version, ", extensions:" if extensions else ""
        )
        ...
        try:
            self.send_packet(FXP_VERSION, None, UInt32(self._version), *sent_extensions)
        except SFTPError as exc:
            await self._cleanup(exc)
            return
        ...
        if self._version == 3:
            ...
            client_version = cast(
                str, self._reader.get_extra_info("client_version", "")
            )

            if any(name in client_version for name in self._nonstandard_symlink_impls):
                self.logger.debug1(
                    "Adjusting for non-standard symlink " "implementation"
                )
                self._nonstandard_symlink = True
...
class SFTPServer:
    ...
    def __init__(self, chan: "SSHServerChannel", chroot: Optional[bytes] = None):
        self._chan = chan
    ...
    async def convert_attrs(self, result: MaybeAwait[_SFTPOSAttrs]) -> SFTPAttrs:
    ...
    def format_longname(self, name: SFTPName) -> MaybeAwait[None]:
        ...
        detail = (
            f"{mode:10s} {nlink:>4s} {user:8s} {group:8s} " f"{size:>8s} {modtime:12s} "
        )
    ...
    def open(self, path: bytes, pflags: int, attrs: SFTPAttrs) -> MaybeAwait[object]:
        ...
        file_obj = open(
            _to_local_path(self.map_path(path)),
            mode,
            buffering=0,
    ...
    def open56(
        ...
        if desired_access & ACE4_READ_DATA and desired_access & ACE4_WRITE_DATA:
    ...
    def read(self, file_obj: object, offset: int, size: int) -> MaybeAwait[bytes]:
    ...
    def write(self, file_obj: object, offset: int, data: bytes) -> MaybeAwait[int]:
    ...
    def lsetstat(self, path: bytes, attrs: SFTPAttrs) -> MaybeAwait[None]:
        ...
        _setstat(_to_local_path(self.map_path(path)), attrs, follow_symlinks=False)
    ...
    def readlink(self, path: bytes) -> MaybeAwait[bytes]:
        ...
        if sys.platform == "win32" and path.startswith("\\\\?\\"):  # pragma: no cover
            path = path[4:]
    ...
    def lock(
        self, file_obj: object, offset: int, length: int, flags: int
    ) -> MaybeAwait[None]:
        """Acquire a byte range lock on an open file"""

        raise SFTPOpUnsupported("Byte range locks not supported")

    def unlock(self, file_obj: object, offset: int, length: int) -> MaybeAwait[None]:
...
class LocalFS:
    ...
    async def stat(self, path: bytes, *, follow_symlinks: bool = True) -> "SFTPAttrs":
        ...
        return SFTPAttrs.from_local(
            os.stat(_to_local_path(path), follow_symlinks=follow_symlinks)
        )
    ...
    @async_context_manager
    async def open(self, path: bytes, mode: str, block_size: int = -1) -> LocalFile:
        """Open a local file"""
...
async def _sftp_handler(
    sftp_server: MaybeAwait[SFTPServer],
    reader: "SSHReader[bytes]",
    writer: "SSHWriter[bytes]",
    sftp_version: int,
) -> None:
