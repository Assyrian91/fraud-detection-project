...
SSHSocketSessionFactory = Callable[["SSHReader", "SSHWriter"], MaybeAwait[None]]
_OptSocketSessionFactory = Optional[SSHSocketSessionFactory]

SSHServerSessionFactory = Callable[
    ["SSHReader", "SSHWriter", "SSHWriter"], MaybeAwait[None]
]
...
class SSHReader(Generic[AnyStr]):
    ...
    async def readuntil(self, separator: object, max_separator_len=0) -> AnyStr:
        ...
        return await self._session.readuntil(
            separator, self._datatype, max_separator_len
        )
...
class SSHStreamSession(Generic[AnyStr]):
    ...
    async def read(self, datatype: DataType, n: int, exact: bool) -> AnyStr:
        ...
        if n > 0 and exact:
            raise asyncio.IncompleteReadError(cast(bytes, result), len(result) + n)
    ...
    async def readuntil(
        self, separator: object, datatype: DataType, max_separator_len=0
    ) -> AnyStr:
...
class SSHClientStreamSession(SSHStreamSession[AnyStr], SSHClientSession[AnyStr]):
    """SSH client stream session handler"""


class SSHServerStreamSession(SSHStreamSession[AnyStr], SSHServerSession[AnyStr]):
    """SSH server stream session handler"""

    def __init__(
        self,
        session_factory: _OptServerSessionFactory,
    ...
    def exec_requested(self, command: str) -> bool:
        ...
        return (self._allow_scp and command.startswith("scp ")) or bool(
    ...
    def session_started(self) -> None:
        ...
        if self._chan.get_subsystem() == "sftp":
            ...
            handler = run_sftp_server(
                self._init_sftp_server(), stdin_bytes, stdout_bytes, self._sftp_version
            )
    ...
    def terminal_size_changed(
        ...
        self.exception_received(TerminalSizeChanged(width, height, pixwidth, pixheight))
...
class SSHTCPStreamSession(SSHSocketStreamSession[AnyStr], SSHTCPSession[AnyStr]):
    """TCP stream session handler"""


class SSHUNIXStreamSession(SSHSocketStreamSession[AnyStr], SSHUNIXSession[AnyStr]):
...
class SSHTunTapStreamSession(SSHSocketStreamSession[bytes], SSHTunTapSession):
    ...
    async def read(self, datatype: DataType, n: int = -1, exact: bool = False) -> bytes:
