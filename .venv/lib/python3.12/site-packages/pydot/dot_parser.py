...
def add_elements(g: Any, toks: ParseResults) -> None:
    for element in toks:
        if isinstance(element, (pydot.core.Subgraph, pydot.core.Cluster)):
            ...
            default_node = pydot.core.Node(element.default_type, **element.attrs)
...
class GraphParser:
    ...
    concat_string = DelimitedList(double_quoted, delim="+", min=2, combine=False)
    ...
    attr_list = OneOrMore(lbrack.suppress() + Optional(a_list) + rbrack.suppress())
    node_stmt = (
        node_id("name") + Optional(attr_list("attr_l")) + Optional(semi.suppress())
    )
    ...
    subgraph = subgraph_("keyword") + Optional(ID("id")) + graph_stmt("contents")

    edgeop = Literal("--") | Literal("->")
    edge_point = subgraph | graph_stmt | node_id
    edge_stmt = DelimitedList(edge_point, delim=edgeop, min=2)("endpoints") + Optional(
        attr_list("attr_l")
    )

    assignment = ID + equals + righthand_id

    stmt = assignment | edge_stmt | default_stmt | subgraph | graph_stmt | node_stmt
