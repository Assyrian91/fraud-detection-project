...
id_re_alpha_nums_with_ports = re.compile(r'^[_a-zA-Z][a-zA-Z0-9_:"]*[a-zA-Z0-9_"]+$')
...
def quote_id_if_necessary(
    ...
    unquoted = [w.lower() for w in list(unquoted_keywords if unquoted_keywords else [])]
...
class Common:
    ...
    def formatted_attr_list(self) -> list[str]:
        ...
        return [self._format_attr(k, v) for k, v in self.obj_dict["attributes"].items()]
...
class Edge(Common):
    ...
    def to_string(self, indent: Any = "", indent_level: int = 1) -> str:
        ...
        if isinstance(src, FrozenDict):
            ...
            edge = [
                sgraph.to_string(indent=indent, indent_level=indent_level, inline=True)
            ]
        else:
            edge = [str(src)]
...
class Graph(Common):
    ...
    def to_string(
        ...
        if edge_obj_dicts:
            edge_ep_set = set(
                itertools.chain.from_iterable(obj["points"] for obj in edge_obj_dicts)
            )
        else:
            edge_ep_set = set()
        ...
        for idx, obj in obj_list:
            if obj["type"] == "node":
                ...
                node_str = node.to_string(indent=indent, indent_level=indent_level + 1)
                ...
                edge_str = edge.to_string(indent=indent, indent_level=indent_level + 1)
...
class Dot(Graph):
    ...
    def create(
        ...
        assert process.returncode == 0, (
            f'"{prog}" with args {arguments} ' f"returned code: {process.returncode}"
        )
