...
URL = "https://iterative-telemetry.herokuapp.com" "/api/v1/s2s/event?ip_policy=strict"
...
class IterativeTelemetryLogger:
    ...
    def event_scope(self, interface: str, action: str) -> Iterator[TelemetryEvent]:
    ...
    def log(
        ...
        def decorator(func):
            ...
            def inner(*args, **kwargs):
                with self.event_scope(interface, action or func.__name__) as event:
                    try:
    ...
    def send_cli_call(self, cmd_name: str, error: Optional[str] = None, **kwargs):
...
def find_or_create_user_id():
    ...
    """
    config_file = Path(user_config_dir(os.path.join("iterative", "telemetry"), False))
    config_file.parent.mkdir(mode=0o755, parents=True, exist_ok=True)
    lockfile = str(config_file.with_suffix(".lock"))
    # DVC backwards-compatibility
    config_file_old = Path(user_config_dir(os.path.join("dvc", "user_id"), "iterative"))
